...
// 무리로 놓는 게 핵심이다. 하나씩 흩으면 개수를 아무리 늘려도 휑해 보인다.
{
  ...
  glades.forEach((g, i) => {
    ...
    for (let k = 0; k < around; k++) {
      ...
      // 돌은 원본을 써도 2천 삼각형이라 부담이 없고, 빌보드가 아니라 진짜 입체라
      // 눈높이에서 봐도 버틴다. 다섯에 하나 꼴로 섞어 빌보드만 늘어놓지 않는다.
      const kind = rand() < 0.2 ? (rand() < 0.5 ? "stones" : "boulder") : "far-bush";
...
{
  // 화분·구슬등을 섞었더니 길가에만 원본 소품이 40개 늘어 20만 삼각형이 됐다.
  // 길가는 지나가며 스치는 자리라 덤불 빌보드로 충분하고, 돌은 원본이라도
  // 2~3천이라 부담이 없어 다섯에 하나꼴로 섞어 납작한 티를 지운다.
  const KINDS = ["far-bush", "far-bush", "stones", "far-bush", "far-bush", "boulder"];
