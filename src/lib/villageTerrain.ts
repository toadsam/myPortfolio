...
export const PLAZA_RING = {
   ...
   * 데크 높이 — **광장 단상과 같아야 한다.**
   *
   * 한동안 0.14 였다(수면 0.03 과 물결 너울 ±0.045 위에 얹는 최소 높이). 그런데
   * 단상은 1.1 이라 광장 가장자리를 밟는 순간 **0.96 이 뚝 떨어졌다** — 캐릭터 키가
   * 0.8 이니 제 키보다 깊게 꺼지는 낙차다. 뛰어오던 NPC 가 광장 테두리에서 갑자기
   * 사라지듯 빠지던 것이 이것이다(모든 각도에서 r≈9.0 에 −0.96, 실측 프로파일).
   *
   * 고리는 단상 축대 **발치**가 아니라 단상과 이어진 **테라스**다. 물과의 낙차는
   * 바깥 모서리 한 줄에 몰리고, 거기는 난간(울타리)이 막는다.
   */
  deck: PLAZA_STEP
};
...
export function terrainHeightAt(x: number, z: number): number {
  // **고리 회랑도 지면이다.** 물 위지만 사람이 딛는 데크가 깔려 있다.
  // 이 한 줄을 빼먹었더니 데크를 1.1 로 올렸을 때 회랑 위 난간·가로등만
  // 0 에 남아 통째로 묻혔다 — 걷기(walkHeightAt)는 고리를 아는데 지면 높이는
  // 몰랐던 탓이다. 굽이(relief)를 안 더하는 것도 walkHeightAt 과 같다(데크는 평평).
