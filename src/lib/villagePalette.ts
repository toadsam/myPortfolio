...
export function timePalette(hour: number): VillagePalette {
  if (hour >= 21 || hour < 5) {
    return {
      ...
      fillI: 0.14,
      hSky: "#22345e",
      // 지면 반사광에 온기 한 방울 — 마을 바닥은 등불빛을 되쏘는 면이다.
      // 순수 남색(#141f26)이면 건물 밑동·처마 밑이 얼음장이 된다.
      hGround: "#1e1a15",
      hI: 0.24,
      // 밤은 태양(달)이 약해 형태를 거의 못 만든다. AO가 그 몫을 대신 지므로 제일 세다.
      aoColor: "#080d1e",
      aoI: 2.2,
      // ─── 컨셉의 밤은 "파란 어둠 위에 타는 금빛" ─────────────────────────
      // 밤답게 만드는 건 하늘이 아니라 **대비**다 — 하늘·달빛은 차게 두고,
      // 등불(lamp)과 창문(windowGlow)을 올려 따뜻한 쪽이 화면을 이긴다.
      // lamp 는 WaterGlints 세기·밤 그림자 오프 판정(GLINT_STRENGTH)에도
      // 물려 있다 — 2.6 이상이면 둘 다 이미 포화라 올려도 부작용이 없다.
      lamp: 3.4,
      windowGlow: 1.5,
