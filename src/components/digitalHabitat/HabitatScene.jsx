...
import {layerOrder, layers, projects, systemNodes, simulations} from "./habitatData";
...
function roundedBox(width, height, depth, material, position, rotation, radius) {
  const mesh = new THREE.Mesh(
    new RoundedBoxGeometry(width, height, depth, 5, radius == null ? 0.035 : radius),
    material
  );
...
function makeTube(points, radius, material, segments) {
  ...
  const mesh = new THREE.Mesh(new THREE.TubeGeometry(curve, segments || 80, radius, 10, false), material);
...
function makeIrregularIsland(radiusX, radiusZ, depth, material, salt) {
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    const wobble = 0.84 + seeded(i, salt) * 0.24 + Math.sin(i * 1.7 + salt) * 0.045;
...
function makeIslandEdge(points, y, material) {
  ...
  return new THREE.Mesh(new THREE.TubeGeometry(curve, 180, 0.012, 8, true), material);
}
...
function makePlate(width, depth, material, accentMaterial) {
  ...
  const plate = new THREE.Mesh(new THREE.CylinderGeometry(1, 1, 0.05, 7), material);
  const edge = new THREE.Mesh(new THREE.TorusGeometry(0.98, 0.012, 6, 7), accentMaterial);
  ...
  edge.userData.baseOpacity = accentMaterial.opacity == null ? 0.44 : accentMaterial.opacity;
  group.add(plate);
  group.add(edge);
...
function rememberMaterials(group) {
  ...
  group.traverse(function collect(node) {
    ...
    list.forEach(function each(material) {
      if (material.isMeshStandardMaterial && materials.indexOf(material) === -1) {
...
function addHotspot(world, layer) {
  ...
  const marker = new THREE.Mesh(new THREE.IcosahedronGeometry(0.065, 2), makeBasic(color, 0.88));
  const ring = new THREE.Mesh(new THREE.TorusGeometry(0.23, 0.01, 8, 64), makeBasic(color, 0.34));
...
function addStudioShell(world, materials) {
  ...
  const lowerIsland = makeIrregularIsland(3.12, 1.72, 0.22, materials.islandUnder, 34);
  island.position.y = -0.38;
  lowerIsland.position.set(0.12, -0.78, -0.05);
  lowerIsland.scale.set(0.92, 1, 0.9);

  const edge = makeIslandEdge(island.userData.edgePoints, 0.02, materials.islandEdge);
  const lowerEdge = makeIslandEdge(lowerIsland.userData.edgePoints, -0.56, materials.islandUnderEdge);
  ...
  const glassPanels = [
    {position: [-1.72, 0.52, -1.18], rotation: [-0.04, 0.52, 0.03], width: 1.28},
    {position: [1.46, 0.58, -1.28], rotation: [-0.02, -0.42, -0.02], width: 1.42},
    {position: [0.26, 0.68, -1.72], rotation: [-0.04, 0.04, 0], width: 1.5}
  ];
  glassPanels.forEach(function addGlass(panel, index) {
    const wall = roundedBox(panel.width, 0.68, 0.035, materials.studioGlass.clone(), panel.position, panel.rotation, 0.045);
    const rail = roundedBox(panel.width * 0.86, 0.024, 0.045, index === 1 ? materials.violetStrip : materials.cyanStrip, [panel.position[0], panel.position[1] + 0.28, panel.position[2] + 0.02], panel.rotation, 0.008);
    group.add(wall);
    group.add(rail);
  ...
  const screens = [
    makeScreen("INTERFACE LAB", ["React states", "Motion systems", "Accessible flows"], "#22D3EE", [-1.48, 0.8, -0.98], [-0.08, 0.58, 0.02], 0.58),
    makeScreen("PROJECT SIGNALS", ["MuscleUp", "Campus Foodmap", "TSEROF"], "#8B5CF6", [1.45, 0.86, -1.04], [-0.08, -0.48, -0.02], 0.56),
    makeScreen("SYSTEM FLOW", ["Auth", "API", "Database", "Deploy"], "#00B4D8", [0.04, 0.96, -1.46], [-0.06, 0.02, 0], 0.54)
  ];
  ...
  const energyPaths = [
    [[-3.0, 0.02, 0.72], [-1.56, 0.12, 0.4], [-0.42, 0.14, 0.16], [0, 0.58, 0]],
    [[2.94, 0.03, 0.48], [1.82, 0.14, 0.16], [0.68, 0.18, 0.08], [0, 0.58, 0]],
    [[-0.8, 0.02, -1.72], [-0.32, 0.16, -0.76], [0.14, 0.22, -0.2], [0, 0.58, 0]],
    [[1.2, 0.02, -1.58], [0.72, 0.15, -0.82], [0.28, 0.18, -0.22], [0, 0.58, 0]]
  ];
  energyPaths.forEach(function addPath(points, index) {
    const path = makeTube(points, index % 2 ? 0.011 : 0.009, index % 2 ? materials.violetCable.clone() : materials.cyanCable.clone(), 90);
    path.userData.phase = index * 0.37;
    group.add(path);
    world.animated.studioRibbons.push(path);
  });

  const workbench = roundedBox(1.18, 0.08, 0.42, materials.workbench, [-1.35, 0.11, 0.62], [0, 0.26, 0], 0.035);
  const beaconBase = roundedBox(0.54, 0.08, 0.54, materials.assetPedestal, [2.08, 0.1, 0.98], [0, -0.18, 0], 0.05);
  group.add(workbench);
...
function addAmbientFireflies(world) {
  for (let i = 0; i < 18; i += 1) {
    ...
    const glow = new THREE.Mesh(new THREE.SphereGeometry(0.035 + seeded(i, 90) * 0.028, 16, 10), makeBasic(color, 0.82));
    ...
    if (node.material) {
      ...
      list.forEach(function tune(material) {
        if (material.isMeshStandardMaterial || material.isMeshPhysicalMaterial) {
          ...
          material.roughness = Math.min(material.roughness == null ? 0.42 : material.roughness, 0.5);
          material.metalness = Math.max(material.metalness == null ? 0.18 : material.metalness, 0.18);
        }
      });
    }
  });
...
function addAssetFallback(world, spec) {
  ...
  const base = new THREE.Mesh(new THREE.CylinderGeometry(0.18, 0.22, 0.1, 8), makeMat({color: 0x0f172a, emissive: color, emissiveIntensity: 0.12, roughness: 0.26, metalness: 0.58}));
  const artifact = new THREE.Mesh(new THREE.IcosahedronGeometry(0.2, 3), makeMat({color: color, emissive: color, emissiveIntensity: 0.46, roughness: 0.22, metalness: 0.44}));
  const ring = new THREE.Mesh(new THREE.TorusGeometry(0.27, 0.01, 8, 72), makeBasic(color, 0.42));
    ...
    loader.load(
      ...
      function onLoad(gltf) {
        ...
        if (gltf.animations && gltf.animations.length) {
          ...
          const preferred = gltf.animations.find(function findIdle(clip) {
...
function addLivingCore(world, materials) {
  const group = new THREE.Group();
  const base = new THREE.Mesh(new THREE.CylinderGeometry(0.54, 0.68, 0.1, 72), materials.coreBase);
  const column = new THREE.Mesh(new THREE.CylinderGeometry(0.18, 0.25, 0.72, 48), materials.coreColumn);
  const core = new THREE.Mesh(new THREE.IcosahedronGeometry(0.28, 4), materials.core);
  const halo = new THREE.Mesh(new THREE.SphereGeometry(0.48, 40, 28), materials.coreHalo);
  const rings = [
    new THREE.Mesh(new THREE.TorusGeometry(0.68, 0.012, 10, 110), materials.coreRing),
    new THREE.Mesh(new THREE.TorusGeometry(0.48, 0.01, 10, 110), materials.coreRingAlt),
    new THREE.Mesh(new THREE.TorusGeometry(0.36, 0.008, 10, 110), materials.coreRingGold)
  ];
  ...
  for (let i = 0; i < 24; i += 1) {
    const particle = new THREE.Mesh(new THREE.IcosahedronGeometry(0.016 + seeded(i, 2) * 0.018, 1), particleMaterial.clone());
...
function addInterfaceLayer(world, materials) {
  ...
  const group = new THREE.Group();
  const plate = makePlate(0.9, 0.58, materials.glassPlate, materials.interfaceLine.clone());
  group.add(plate);

  const labFrame = roundedBox(1.0, 0.055, 0.055, materials.cyanStrip.clone(), [0, 0.68, -0.18], [0.02, 0, 0], 0.012);
  const labLeft = roundedBox(0.055, 0.46, 0.055, materials.studioGlass.clone(), [-0.48, 0.46, -0.18], [0.02, 0, 0], 0.014);
  const labRight = roundedBox(0.055, 0.46, 0.055, materials.studioGlass.clone(), [0.48, 0.46, -0.18], [0.02, 0, 0], 0.014);
  group.add(labFrame);
  group.add(labLeft);
  group.add(labRight);
  ...
  panelPositions.forEach(function createPanel(pos, index) {
    const panel = roundedBox(0.42, 0.2, 0.032, index % 2 ? materials.interfacePanelMint : materials.interfacePanel, pos, [0.08, -0.18 + index * 0.12, 0], 0.022);
    const chip = roundedBox(0.1, 0.035, 0.018, materials.interfaceChip, [pos[0] - 0.12, pos[1] + 0.012, pos[2] + 0.03], [0.08, -0.18 + index * 0.12, 0], 0.01);
    const line = roundedBox(0.22, 0.018, 0.018, materials.interfaceScan, [pos[0] + 0.07, pos[1] - 0.035, pos[2] + 0.032], [0.08, -0.18 + index * 0.12, 0], 0.006);
  ...
  const scan = roundedBox(0.86, 0.012, 0.018, materials.interfaceScan, [0, 0.58, 0.32], null, 0.005);
  group.add(scan);
...
function addWorksLayer(world, materials) {
  ...
  const group = new THREE.Group();
  const plate = makePlate(1.08, 0.66, materials.glassPlate, materials.worksLine.clone());
  group.add(plate);

  const galleryBack = roundedBox(1.1, 0.38, 0.035, materials.studioGlass.clone(), [0.05, 0.56, -0.24], [0.02, 0, 0], 0.04);
  const galleryRail = roundedBox(1.0, 0.026, 0.04, materials.violetStrip.clone(), [0.05, 0.75, -0.22], [0.02, 0, 0], 0.008);
  ...
  projects.forEach(function createProject(project, index) {
    const mat = index === 0 ? materials.projectMint : index === 1 ? materials.projectGold : index === 2 ? materials.projectBlue : materials.projectPink;
    const module = roundedBox(0.22, 0.18, 0.18, mat, positions[index], [0.22, 0.4 + index * 0.32, 0.18], 0.04);
    const orbit = new THREE.Mesh(new THREE.TorusGeometry(0.16, 0.006, 8, 48), mat.clone());
    const plinth = new THREE.Mesh(new THREE.CylinderGeometry(0.14, 0.18, 0.08, 6), materials.assetPedestal.clone());
    const miniPanel = roundedBox(0.24, 0.05, 0.018, mat.clone(), [positions[index][0], positions[index][1] - 0.16, positions[index][2] + 0.02], null, 0.012);
    orbit.position.copy(module.position);
    orbit.rotation.y = Math.PI / 2;
    plinth.position.set(positions[index][0], positions[index][1] - 0.16, positions[index][2]);
...
function addSystemLayer(world, materials) {
  ...
  const group = new THREE.Group();
  const plate = makePlate(1.14, 0.72, materials.deepSystemPlate, materials.systemLine.clone());
  group.add(plate);

  const rack = roundedBox(0.34, 0.66, 0.22, materials.apiNode.clone(), [-0.58, 0.52, -0.12], [0, 0.08, 0], 0.04);
  group.add(rack);
  for (let i = 0; i < 5; i += 1) {
    const serverLine = roundedBox(0.26, 0.026, 0.026, materials.systemLed.clone(), [-0.58, 0.3 + i * 0.085, 0.02], [0, 0.08, 0], 0.006);
  ...
  systemNodes.forEach(function createSystemNode(node, index) {
    const isDb = index === 2;
    const mesh = isDb
      ? new THREE.Mesh(new THREE.CylinderGeometry(0.12, 0.12, 0.18, 32), materials.dbCore)
      : roundedBox(0.22, 0.13, 0.16, index === 0 ? materials.authGate : materials.apiNode, nodePositions[index], null, 0.035);
    if (isDb) {
      mesh.position.fromArray(nodePositions[index]);
    }
    const led = roundedBox(0.05, 0.018, 0.018, materials.systemLed, [nodePositions[index][0], nodePositions[index][1] + 0.085, nodePositions[index][2] + 0.09], null, 0.006);
  ...
  const shield = new THREE.Mesh(
    new THREE.ExtrudeGeometry(shieldShape, {depth: 0.04, bevelEnabled: true, bevelSegments: 2, bevelSize: 0.01, bevelThickness: 0.01}),
...
function addSimulationLayer(world, materials) {
  ...
  const group = new THREE.Group();
  const plate = makePlate(0.96, 0.7, materials.simPlate, materials.simLine.clone());
  group.add(plate);

  const room = roundedBox(0.62, 0.46, 0.46, materials.simChamber, [0, 0.52, 0], null, 0.035);
  room.material.wireframe = true;
  const portalOuter = new THREE.Mesh(new THREE.TorusGeometry(0.36, 0.024, 10, 92), materials.simLine.clone());
  const portalInner = new THREE.Mesh(new THREE.TorusGeometry(0.24, 0.011, 8, 92), materials.projectPink.clone());
  const marker = new THREE.Mesh(new THREE.RingGeometry(0.18, 0.27, 4), materials.arMarker);
  const target = new THREE.Mesh(new THREE.IcosahedronGeometry(0.11, 1), materials.targetPink);
  const headset = roundedBox(0.34, 0.14, 0.16, materials.xrHeadset, [0.44, 0.42, 0.12], [0, -0.22, 0], 0.038);
  ...
  simulations.forEach(function addSim(sim, index) {
    const node = roundedBox(0.16, 0.05, 0.05, index === 1 ? materials.projectPink : materials.projectBlue, [-0.32 + index * 0.28, 0.18, -0.32], null, 0.015);
    group.add(node);
  });
...
function addEvolutionLayer(world, materials) {
  ...
  for (let i = 0; i < 7; i += 1) {
    const x = -1.28 + i * 0.43;
    const node = roundedBox(0.08, 0.13, 0.08, i % 2 ? materials.evolutionMint : materials.evolutionLife, [x, 0.35 + Math.sin(i) * 0.04, Math.sin(i * 1.6) * 0.16], null, 0.025);
...
function addSignalLayer(world, materials) {
  ...
  const group = new THREE.Group();
  const orb = new THREE.Mesh(new THREE.IcosahedronGeometry(0.16, 3), materials.signalOrb);
  const satellite = roundedBox(0.26, 0.08, 0.16, materials.signalBody, [0.34, 0.08, 0.02], [0.1, -0.28, 0], 0.025);
  const mast = new THREE.Mesh(new THREE.CylinderGeometry(0.016, 0.022, 0.62, 18), materials.signalMast);
  const waveA = new THREE.Mesh(new THREE.TorusGeometry(0.36, 0.006, 8, 72), materials.signalWave);
  const waveB = new THREE.Mesh(new THREE.TorusGeometry(0.58, 0.005, 8, 72), materials.signalWave);
  const beam = new THREE.Mesh(new THREE.CylinderGeometry(0.012, 0.052, 1.34, 24, 1, true), materials.signalWave.clone());
  const dish = new THREE.Mesh(new THREE.ConeGeometry(0.22, 0.1, 40, 1, true), materials.studioGlass.clone());
  orb.position.y = 0.38;
...
function addDataLines(world, materials) {
  layers.forEach(function createLine(layer) {
    ...
    const pulse = new THREE.Mesh(new THREE.IcosahedronGeometry(0.032, 1), makeBasic(color, 0.54));
...
function createMaterials() {
  return {
    islandTop: makeMat({color: 0x050816, emissive: 0x0b2444, emissiveIntensity: 0.05, roughness: 0.66, metalness: 0.18}),
    islandUnder: makeMat({color: 0x020617, emissive: 0x1e1b4b, emissiveIntensity: 0.06, roughness: 0.72, metalness: 0.16}),
    islandEdge: makeMat({color: 0x38bdf8, emissive: 0x38bdf8, emissiveIntensity: 0.8, transparent: true, opacity: 0.45, roughness: 0.2}),
    islandUnderEdge: makeMat({color: 0x8b5cf6, emissive: 0x8b5cf6, emissiveIntensity: 0.48, transparent: true, opacity: 0.25, roughness: 0.2}),
    studioGlass: makeMat({color: 0x0b1020, emissive: 0x38bdf8, emissiveIntensity: 0.08, transparent: true, opacity: 0.23, roughness: 0.05, metalness: 0.42, side: THREE.DoubleSide}),
    workbench: makeMat({color: 0x101827, emissive: 0x22d3ee, emissiveIntensity: 0.07, roughness: 0.28, metalness: 0.62}),
    assetPedestal: makeMat({color: 0x0f172a, emissive: 0xfacc15, emissiveIntensity: 0.1, roughness: 0.24, metalness: 0.56}),
    cyanStrip: makeMat({color: 0x22d3ee, emissive: 0x22d3ee, emissiveIntensity: 0.8, transparent: true, opacity: 0.64}),
    violetStrip: makeMat({color: 0x8b5cf6, emissive: 0x8b5cf6, emissiveIntensity: 0.75, transparent: true, opacity: 0.58}),
    cyanCable: makeMat({color: 0x38bdf8, emissive: 0x38bdf8, emissiveIntensity: 0.72, transparent: true, opacity: 0.32}),
    violetCable: makeMat({color: 0xa855f7, emissive: 0xa855f7, emissiveIntensity: 0.68, transparent: true, opacity: 0.28}),
    glassPlate: makeMat({color: 0x0b1020, emissive: 0x111827, emissiveIntensity: 0.08, transparent: true, opacity: 0.58, roughness: 0.18, metalness: 0.28}),
    deepSystemPlate: makeMat({color: 0x07112a, emissive: 0x1e3a8a, emissiveIntensity: 0.08, transparent: true, opacity: 0.55, roughness: 0.22, metalness: 0.36}),
    simPlate: makeMat({color: 0x130b24, emissive: 0xa855f7, emissiveIntensity: 0.05, transparent: true, opacity: 0.5, roughness: 0.2, metalness: 0.28}),
    coreBase: makeMat({color: 0x111827, emissive: 0x38bdf8, emissiveIntensity: 0.12, roughness: 0.34, metalness: 0.55}),
    coreColumn: makeMat({color: 0x38bdf8, emissive: 0x38bdf8, emissiveIntensity: 0.14, transparent: true, opacity: 0.16, roughness: 0.08}),
    core: makeMat({color: 0x7dd3fc, emissive: 0x38bdf8, emissiveIntensity: 1.05, roughness: 0.16}),
    coreHalo: makeMat({color: 0x38bdf8, emissive: 0x38bdf8, emissiveIntensity: 0.48, transparent: true, opacity: 0.08, side: THREE.DoubleSide, depthWrite: false}),
    coreRing: makeMat({color: 0x38bdf8, emissive: 0x38bdf8, emissiveIntensity: 1.1, transparent: true, opacity: 0.72}),
    coreRingAlt: makeMat({color: 0x8b5cf6, emissive: 0x8b5cf6, emissiveIntensity: 0.95, transparent: true, opacity: 0.66}),
    coreRingGold: makeMat({color: 0xfacc15, emissive: 0xfacc15, emissiveIntensity: 0.55, transparent: true, opacity: 0.5}),
    interfaceLine: makeMat({color: 0x22d3ee, emissive: 0x22d3ee, emissiveIntensity: 1.0, transparent: true, opacity: 0.48}),
    interfacePanel: makeMat({color: 0x0f2740, emissive: 0x22d3ee, emissiveIntensity: 0.26, transparent: true, opacity: 0.74, roughness: 0.16}),
    interfacePanelMint: makeMat({color: 0x0d2f35, emissive: 0x2dd4bf, emissiveIntensity: 0.24, transparent: true, opacity: 0.72, roughness: 0.16}),
    interfaceChip: makeMat({color: 0xf8fafc, emissive: 0x38bdf8, emissiveIntensity: 0.2, transparent: true, opacity: 0.7}),
    interfaceScan: makeMat({color: 0x2dd4bf, emissive: 0x2dd4bf, emissiveIntensity: 1.2, transparent: true, opacity: 0.8}),
    worksLine: makeMat({color: 0x8b5cf6, emissive: 0x8b5cf6, emissiveIntensity: 0.95, transparent: true, opacity: 0.5}),
    projectMint: makeMat({color: 0x22d3ee, emissive: 0x2dd4bf, emissiveIntensity: 0.32, transparent: true, opacity: 0.76}),
    projectGold: makeMat({color: 0x38bdf8, emissive: 0xfacc15, emissiveIntensity: 0.26, transparent: true, opacity: 0.74}),
    projectBlue: makeMat({color: 0x8b5cf6, emissive: 0x38bdf8, emissiveIntensity: 0.3, transparent: true, opacity: 0.76}),
    projectPink: makeMat({color: 0xa855f7, emissive: 0xec4899, emissiveIntensity: 0.32, transparent: true, opacity: 0.74}),
    systemLine: makeMat({color: 0x00b4d8, emissive: 0x00b4d8, emissiveIntensity: 0.95, transparent: true, opacity: 0.5}),
    apiNode: makeMat({color: 0x1e3a8a, emissive: 0x00b4d8, emissiveIntensity: 0.24, roughness: 0.32, metalness: 0.5}),
    authGate: makeMat({color: 0x0f2740, emissive: 0x38bdf8, emissiveIntensity: 0.3, roughness: 0.28, metalness: 0.5}),
    dbCore: makeMat({color: 0x0f172a, emissive: 0x00b4d8, emissiveIntensity: 0.42, roughness: 0.24, metalness: 0.48}),
    systemLed: makeMat({color: 0x7dd3fc, emissive: 0x00b4d8, emissiveIntensity: 1.2, transparent: true, opacity: 0.8}),
    securityShield: makeMat({color: 0x00b4d8, emissive: 0x00b4d8, emissiveIntensity: 0.42, transparent: true, opacity: 0.48}),
    simLine: makeMat({color: 0xa855f7, emissive: 0xa855f7, emissiveIntensity: 0.9, transparent: true, opacity: 0.5}),
    simChamber: makeMat({color: 0x8b5cf6, emissive: 0xa855f7, emissiveIntensity: 0.42, transparent: true, opacity: 0.42}),
    arMarker: makeBasic(0x38bdf8, 0.52),
    targetPink: makeMat({color: 0xec4899, emissive: 0xec4899, emissiveIntensity: 0.82, transparent: true, opacity: 0.82}),
    xrHeadset: makeMat({color: 0x151225, emissive: 0xa855f7, emissiveIntensity: 0.16, roughness: 0.28, metalness: 0.5}),
    evolutionStream: makeMat({color: 0x34d399, emissive: 0x2dd4bf, emissiveIntensity: 0.72, transparent: true, opacity: 0.62}),
    evolutionMint: makeMat({color: 0x2dd4bf, emissive: 0x2dd4bf, emissiveIntensity: 0.5}),
    evolutionLife: makeMat({color: 0x34d399, emissive: 0x34d399, emissiveIntensity: 0.48}),
    signalOrb: makeMat({color: 0xe0f2fe, emissive: 0x38bdf8, emissiveIntensity: 1.35, roughness: 0.16}),
    signalBody: makeMat({color: 0x111827, emissive: 0xfacc15, emissiveIntensity: 0.18, roughness: 0.3, metalness: 0.58}),
    signalMast: makeMat({color: 0xe0f2fe, emissive: 0x38bdf8, emissiveIntensity: 0.26, roughness: 0.24, metalness: 0.6}),
    signalWave: makeMat({color: 0xfacc15, emissive: 0xfacc15, emissiveIntensity: 0.65, transparent: true, opacity: 0.5})
  };
}
...
function createHabitat(scene) {
  ...
  layers.forEach(function prepareHotspot(layer) {
    layer.hotspotRadius = layer.id === "evolution" ? 0.88 : layer.id === "works" ? 0.72 : 0.62;
  ...
  world.animated.particles = new THREE.Points(
    particleGeometry,
    new THREE.PointsMaterial({color: 0x7dd3fc, size: 0.022, transparent: true, opacity: 0.5, depthWrite: false})
  );
...
function HabitatScene(props) {
  ...
  useEffect(function syncActive() {
    activeRef.current = activeLayer;
  }, [activeLayer]);

  useEffect(function syncHover() {
    hoverRef.current = hoveredLayer;
  }, [hoveredLayer]);

  useEffect(function syncHandlers() {
  ...
  useEffect(function mountScene() {
    ...
    const bloomPass = new UnrealBloomPass(new THREE.Vector2(1, 1), 0.26, 0.48, 0.88);
    ...
    function updateMaterialGlow(group, amount) {
      ...
      group.userData.materials.forEach(function update(material) {
        ...
        material.emissiveIntensity += (base + amount - material.emissiveIntensity) * 0.12;
      });
    }

    function animate(time) {
      const elapsed = time * 0.001;
      const delta = Math.min(0.05, previousElapsed ? elapsed - previousElapsed : 0.016);
      ...
      if (world.animated.studioShell) {
        world.animated.studioShell.rotation.y = Math.sin(elapsed * 0.11) * 0.018;
      }

      world.animated.studioPanels.forEach(function animateStudioPanel(panel, index) {
        if (panel.userData.baseY == null) {
          panel.userData.baseY = panel.position.y;
        }
        panel.position.y = panel.userData.baseY + Math.sin(elapsed * 0.9 + index * 1.4) * 0.012;
        panel.material.opacity = 0.18 + (Math.sin(elapsed * 1.1 + index) * 0.5 + 0.5) * 0.08;
      });

      world.animated.studioScreens.forEach(function animateScreen(screen, index) {
        screen.position.y = screen.userData.baseY + Math.sin(elapsed * 0.85 + index) * 0.018;
        screen.material.opacity = 0.78 + (Math.sin(elapsed * 1.25 + index) * 0.5 + 0.5) * 0.16;
      });

      world.animated.studioRibbons.forEach(function animateRibbon(ribbon, index) {
        if (ribbon.material && ribbon.material.opacity != null) {
          ribbon.material.opacity = 0.18 + (Math.sin(elapsed * 1.05 + index) * 0.5 + 0.5) * 0.36;
        }
      });

      world.animated.fireflies.forEach(function animateFirefly(fly, index) {
        const angle = fly.userData.seed + elapsed * fly.userData.speed + Math.sin(elapsed * 0.17 + index) * 0.3;
        const radius = fly.userData.radius + Math.sin(elapsed * 0.23 + index) * 0.24;
        fly.position.set(
      ...
      world.animated.loadedAssets.forEach(function animateLoadedAsset(asset, index) {
        asset.position.y = asset.userData.baseY + Math.sin(elapsed * 0.72 + index) * 0.025;
      ...
      if (world.animated.core) {
        ...
        const pulse = 1 + Math.sin(elapsed * animationConfig.corePulseSpeed) * 0.085;
        core.scale.setScalar(pulse);
        halo.scale.setScalar(1.02 + Math.sin(elapsed * 2.05) * 0.075);
        halo.material.opacity = 0.055 + (Math.sin(elapsed * 2.05) * 0.5 + 0.5) * 0.055;
        rings[0].rotation.z += 0.009;
        rings[1].rotation.x += 0.007;
        rings[2].rotation.z -= 0.005;
      }

      world.animated.coreParticles.forEach(function animateCoreParticle(particle, index) {
      ...
      world.animated.interfacePanels.forEach(function animatePanel(panel, index) {
        panel.position.y = panel.userData.baseY + Math.sin(elapsed * 1.25 + index) * 0.018;
        panel.rotation.z = Math.sin(elapsed * 0.8 + index) * 0.012;
      });

      if (world.layerGroups.interface && world.layerGroups.interface.userData.scan) {
        const scan = world.layerGroups.interface.userData.scan;
        scan.position.x = Math.sin(elapsed * 1.2) * 0.24;
      }

      world.animated.projectModules.forEach(function animateProject(module, index) {
        module.position.y = module.userData.baseY + Math.sin(elapsed * 1.45 + index) * 0.038;
        module.rotation.y += 0.008 + index * 0.001;
      });

      world.animated.systemNodes.forEach(function animateSystemNode(node, index) {
        node.position.y += Math.sin(elapsed * 1.7 + index) * 0.0006;
      });

      world.animated.systemLeds.forEach(function animateLed(led, index) {
        const blink = Math.sin(elapsed * (2.9 + index * 0.2) + led.userData.seed) * 0.5 + 0.5;
        led.material.opacity = 0.36 + blink * 0.54;
        led.material.emissiveIntensity = 0.7 + blink * 1.0;
      });

      if (world.layerGroups.simulation && world.layerGroups.simulation.userData.target) {
        const target = world.layerGroups.simulation.userData.target;
        target.position.y = target.userData.baseY + Math.sin(elapsed * 1.8) * 0.055;
      ...
      world.animated.portalRings.forEach(function animatePortal(ring, index) {
        ...
        ring.material.opacity = 0.42 + (Math.sin(elapsed * 1.5 + index) * 0.5 + 0.5) * 0.26;
      });

      world.animated.evolutionNodes.forEach(function animateEvolution(node, index) {
        node.position.y = node.userData.baseY + Math.sin(elapsed * 1.35 + index) * 0.03;
      ...
      if (world.layerGroups.signal) {
        ...
        waves.forEach(function animateWave(wave, index) {
          wave.scale.setScalar(1 + (Math.sin(elapsed * 1.4 + index) * 0.5 + 0.5) * 0.18);
          wave.material.opacity = 0.22 + (Math.sin(elapsed * 1.4 + index) * 0.5 + 0.5) * 0.32;
        });
      }

      world.animated.dataPulses.forEach(function animatePulse(item, index) {
        const isActive = selected === item.layerId;
        const isHover = activeHover === item.layerId || hoverRef.current === item.layerId;
        const t = (elapsed * (isActive ? 0.22 : 0.12) + item.phase + index * 0.03) % 1;
      ...
      layerOrder.forEach(function updateLayer(layerId) {
        ...
        const scale = isActive ? animationConfig.activeScale : isHover ? animationConfig.hoverScale : 1;
        if (group) {
          const baseY = group.userData.baseY || 0;
          group.position.y = baseY + Math.sin(elapsed * 1.5 + layerId.length) * (isActive ? 0.065 : isHover ? 0.045 : 0.018);
          group.scale.lerp(tempScale.set(scale, scale, scale), 0.1);
          updateMaterialGlow(group, isActive ? 0.22 : isHover ? 0.13 : 0);
          if (group.children[0] && group.children[0].userData && group.children[0].userData.edge) {
            const edge = group.children[0].userData.edge;
            edge.material.opacity += ((isActive ? 0.85 : isHover ? 0.64 : edge.userData.baseOpacity || 0.42) - edge.material.opacity) * 0.12;
            edge.rotation.z += isActive || isHover ? 0.01 : 0.003;
          }
        }
        if (marker) {
          marker.position.y = marker.userData.baseY + Math.sin(elapsed * 1.2 + layerId.length) * 0.025;
          marker.userData.ring.rotation.z += isActive || isHover ? 0.018 : 0.006;
          marker.userData.marker.material.opacity += ((isActive ? 1 : isHover ? 0.9 : 0.42) - marker.userData.marker.material.opacity) * 0.12;
          marker.userData.ring.material.opacity += ((isActive ? 0.86 : isHover ? 0.62 : 0.22) - marker.userData.ring.material.opacity) * 0.12;
          marker.userData.label.material.opacity += ((isActive || isHover ? 0.96 : 0) - marker.userData.label.material.opacity) * 0.14;
          marker.scale.lerp(tempScale.set(isActive ? 1.7 : isHover ? 1.38 : 1, isActive ? 1.7 : isHover ? 1.38 : 1, isActive ? 1.7 : isHover ? 1.38 : 1), 0.12);
        }
        if (world.lines[layerId]) {
          world.lines[layerId].material.opacity += ((isActive ? 0.74 : isHover ? 0.48 : 0.14) - world.lines[layerId].material.opacity) * 0.12;
        }
      });
  ...
  return <div className="habitat-scene" ref={mountRef} aria-label="Interactive digital habitat scene" />;
}
