        ...
        >
          <div
            style={{
              ...
              border: `1.5px solid ${hovered ? "#ff5a3a" : "#e0604070"}`,
              borderRadius: 8,
              padding: "4px 12px",
              whiteSpace: "nowrap",
              boxShadow: hovered ? "0 0 14px #e0604055" : "none"
            }}
          >
            {hovered ? "메일 보내기 →" : "우편함"}
          </div>
...
function LinkSign({
  ...
  onRipple: (pos: [number, number, number], color: string) => void;
}) {
  const [hovered, setHovered] = useState(false);
  const [shouldShow, setShouldShow] = useState(false);
  const groupRef = useRef<Group>(null);
    ...
    >
      <mesh castShadow>
        ...
        <meshStandardMaterial
          ...
          emissive={color}
          emissiveIntensity={hovered ? 0.14 : 0.03}
          roughness={0.45}
        />
      </mesh>
      ...
      >
        ...
        >
          ...
          <div style={{fontSize: 9, color: "#5e6757", marginTop: 3}}>
            {link.value}
          </div>
          {hovered ? (
            <div
    ...
    <group>
      ...
      {[-3, -1, 1, 3].map(x =>
        [-3, -1, 1, 3].map(z => (
          <mesh
            key={`${x}-${z}`}
            position={[x * 1.4, 0.005, z * 1.4]}
            rotation={[-Math.PI / 2, 0, 0]}
          >
            <planeGeometry args={[1.3, 1.3]} />
            <meshBasicMaterial
              color={((x + z) % 2 === 0 ? "#e8e0d8" : "#f4f0ea") as string}
              transparent
              opacity={0.8}
            />
          </mesh>
...
export function PostInterior({onBack}: Props) {
  ...
  return (
    <div className="fixed inset-0 z-40 bg-[#f8f5f0]">
      ...
      <motion.div
        {...uiDelay(-1)}
        className="pointer-events-none fixed left-1/2 top-4 z-50 -translate-x-1/2 rounded-xl border border-[#e06040]/40 bg-white/80 px-5 py-2 text-xs font-black uppercase tracking-[0.22em] text-[#e06040] backdrop-blur-md"
      ...
      <motion.div
        {...uiDelay(1)}
        className="pointer-events-none fixed bottom-5 left-1/2 z-50 flex -translate-x-1/2 items-center gap-2 rounded-xl border border-[#e06040]/30 bg-white/80 px-4 py-2.5 backdrop-blur-md"
        ...
        {showMailModal ? (
          ...
          >
            ...
            >
              ...
              >
                <p className="text-xs font-black uppercase tracking-[0.2em] text-[#e06040]">
                  Contact
                </p>
                <h2 className="mt-2 text-2xl font-black text-[#1f2a24]">
                  연락하기
                </h2>
                <p className="mt-3 text-sm leading-6 text-[#5e6757]">
                  다음 프로젝트, 인턴십, 협업 제안은 아래 링크로 연락할 수
                  있습니다.
                </p>
                <div className="mt-5 space-y-2.5">
                  {portfolioLinks.map((link, i) => (
                    <a
                      ...
                      rel="noreferrer"
                      style={{
                        borderLeftColor: LINK_COLORS[i % LINK_COLORS.length],
                        borderLeftWidth: 3
                      }}
                      target={
                        link.href.startsWith("mailto:") ? undefined : "_blank"
                    ...
                    >
                      ...
                      <span className="truncate text-sm text-[#5a9857]">
                        {link.value}
                      </span>
                    </a>
                  ))}
