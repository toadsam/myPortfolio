...
function groupAssets(assets: string[]): {category: string; files: string[]}[] {
  ...
  return [...map.entries()].sort((x, y) => x[0].localeCompare(y[0])).map(([category, files]) => ({category, files}));
}

type Selection = {kind: "prop" | "building"; id: string} | null;
type Transform = {position: [number, number, number]; rotationY: number; scale: number};
...
export function usePropsEditor() {
  ...
  const [buildingOverrides, setBuildingOverrides] = useState<Record<string, BuildingOverride>>(initial.buildings ?? {});
  const [selection, setSelection] = useState<Selection>(null);
  const [dragging, setDragging] = useState(false);
  const [status, setStatus] = useState("");
  // 트레이 → 씬 드래그 드롭 요청 (스크린 좌표)
  const [pendingDrop, setPendingDrop] = useState<{glb: string; clientX: number; clientY: number} | null>(null);
  ...
  const selTransform: Transform | null = (() => {
    if (selection?.kind === "prop") {
      ...
      return p ? {position: p.position, rotationY: p.rotationY, scale: p.scale} : null;
    }
    if (selection?.kind === "building") {
      const b = villageBuildings.find((x) => x.id === selection.id);
      if (!b) return null;
      const ov = buildingOverrides[selection.id] ?? {};
      return {position: ov.position ?? b.position, rotationY: ov.rotationY ?? 0, scale: ov.scale ?? 1};
    }
    return null;
  })();
  ...
  function updateSelected(patch: Partial<Transform>) {
    if (selection?.kind === "prop") {
      ...
      setBuildingOverrides((prev) => ({...prev, [selection.id]: {...(prev[selection.id] ?? {}), ...patch}}));
    }
  }
  ...
  async function save() {
    ...
    try {
      ...
      setStatus(res.ok ? `저장 완료 (prop ${d.props} · 건물 ${d.buildings})` : `저장 실패: ${d.error ?? res.status}`);
    } catch (err) {
  ...
  return {
    ...
    assets, items, buildingOverrides,
    selection, selTransform, setSelection,
    dragging, setDragging,
    pendingDrop, setPendingDrop,
    status,
    updateSelected, moveSelected, addPropAt, deleteSelected, save,
  };
}
...
function SelectionCoordLabel({api}: {api: PropsEditorApi}) {
  ...
  return (
    <Html position={[x, labelY, z]} center distanceFactor={9} zIndexRange={[40, 0]}>
      <div
        style={{
          pointerEvents: "none",
          display: "flex",
...
export function PropsEditorTray({api}: {api: PropsEditorApi}) {
  const {enabled, editMode, setEditMode, assets, selection, selTransform, status, updateSelected, deleteSelected, save} = api;
    ...
    <>
      ...
      <button
        ...
        className={`fixed left-4 top-20 z-40 rounded-lg border px-3 py-2 font-mono text-xs font-black backdrop-blur-md transition ${
          editMode ? "border-[#86b0e6] bg-[#5b8fd6]/20 text-[#86b0e6]" : "border-[#5b8fd6]/35 bg-[#0a101b]/85 text-[#5b8fd6]/80 hover:border-[#86b0e6]"
        }`}
      >
        ...
        <>
          ...
          {selTransform ? (
            <div className="fixed right-4 top-20 z-40 flex w-64 flex-col gap-2.5 rounded-xl border border-[#5b8fd6]/30 bg-[#0a101b]/92 p-3.5 font-mono text-[#cdd9ec] shadow-2xl backdrop-blur-md">
              <div className="text-xs font-black uppercase tracking-[0.16em] text-[#86b0e6]">
                {selection?.kind === "building" ? "🏢 건물 편집" : "📦 prop 편집"}
              </div>
              {/* 좌표 직접 입력 (드래그 대신 정밀 배치) */}
              <div className="flex gap-1.5">
                {([0, 1, 2] as const).map((axis) => (
                  <label key={axis} className="flex-1 text-[9px] uppercase tracking-[0.1em] text-[#5a6678]">
                    {["X", "Y", "Z"][axis]}
                    <input
                      type="number" step={0.1} value={Number(selTransform.position[axis].toFixed(2))}
                      onChange={(e) => {
                        const next = [...selTransform.position] as [number, number, number];
              ...
              <div className="text-[9px] text-[#5a6678]">씬에서 드래그 또는 위 칸에 직접 입력</div>
              <label className="text-[10px] uppercase tracking-[0.14em] text-[#5a6678]">
                높이 Y · {selTransform.position[1].toFixed(2)}
                <input type="range" min={-1} max={4} step={0.05} value={selTransform.position[1]}
                  onChange={(e) => updateSelected({position: [selTransform.position[0], Number(e.target.value), selTransform.position[2]]})}
                  className="mt-1 w-full accent-[#86b0e6]" />
              </label>
              <label className="text-[10px] uppercase tracking-[0.14em] text-[#5a6678]">
                회전 · {((selTransform.rotationY * 180) / Math.PI).toFixed(0)}°
                <input type="range" min={0} max={Math.PI * 2} step={0.02} value={selTransform.rotationY}
                  onChange={(e) => updateSelected({rotationY: Number(e.target.value)})} className="mt-1 w-full accent-[#86b0e6]" />
              </label>
              <label className="text-[10px] uppercase tracking-[0.14em] text-[#5a6678]">
                크기 · {selTransform.scale.toFixed(2)}x
                <input type="range" min={0.1} max={6} step={0.05} value={selTransform.scale}
                  onChange={(e) => updateSelected({scale: Number(e.target.value)})} className="mt-1 w-full accent-[#86b0e6]" />
              </label>
              <button type="button" onClick={deleteSelected}
                className="rounded border border-[#ef4444]/40 bg-[#ef4444]/10 px-2 py-1.5 text-[11px] text-[#ff8a80] hover:bg-[#ef4444]/20">
                {selection?.kind === "building" ? "↺ 원위치로 (override 제거)" : "🗑 삭제"}
              </button>
            </div>
          ) : null}
          ...
          <div className="fixed bottom-4 left-1/2 z-40 flex max-h-[40vh] w-[min(92vw,920px)] -translate-x-1/2 flex-col gap-2 rounded-xl border border-[#5b8fd6]/30 bg-[#0a101b]/92 p-3 font-mono text-[#cdd9ec] shadow-2xl backdrop-blur-md">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-black uppercase tracking-[0.16em] text-[#86b0e6]">에셋 트레이 · 씬으로 드래그</span>
            ...
            {assets.length === 0 ? (
              <p className="text-[11px] leading-5 text-[#5a6678]">
                props 폴더에 GLB가 없어요. Meshy GLB를 <span className="text-[#86b0e6]">public/models/props/raw/&lt;카테고리&gt;/</span>에 넣고 <span className="text-[#86b0e6]">npm run optimize</span> 실행.
              </p>
            ) : (
              <div className="flex gap-4 overflow-x-auto pb-1">
                {groupAssets(assets).map(({category, files}) => (
                  <div key={category} className="flex flex-col gap-1">
                    <div className="text-[10px] font-black uppercase tracking-[0.14em] text-[#86b0e6]/70">📁 {category}</div>
                    <div className="flex gap-1.5">
                      {files.map((a) => (
                        <div
                          key={a}
                          draggable
                          onDragStart={(e) => e.dataTransfer.setData("application/x-prop-glb", a)}
                          title={a.split("/").pop()}
                        ...
                        >
                          <span className="line-clamp-2 break-all">{a.split("/").pop()?.replace(".glb", "")}</span>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            {status ? <div className="text-[10px] text-[#86b0e6]">{status}</div> : null}
          </div>
        </>
      ) : null}
    </>
