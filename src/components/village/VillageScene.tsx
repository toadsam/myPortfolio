...
function WaterwaysImpl() {
  ...
  const material = useMemo(() => {
    ...
    applyWaterFlow(m, makeWaterNormal(), {
      ...
      // 안 넘는 최대치다. 속도는 석호(swellSpeed)와 반드시 같은 값으로.
      swellAmp: 0.022,
      swellSpeed: 0.9
    });
    return m;
  ...
  const lagoonMaterial = useMemo(() => {
    const m = new MeshStandardMaterial({
      ...
      roughness: 0.38,
      metalness: 0.12,
      // 0.25 → 0.34: 밤에 달빛 스페큘러가 안 닿는 영역이 **거의 검정**이 되어
      // 밝은 물과 딴 물처럼 갈라져 보였다("끊겨 보인다"의 절반은 톤 단절).
      // 바닥광을 올려 어두운 데도 짙은 물빛으로 남게 한다. 낮에는 햇빛(2.5)에
      // 묻히는 크기다. 리본과 반드시 같은 값으로.
      emissive: new Color("#0f3f57"),
      emissiveIntensity: 0.34
    });
    applyWaterSurface(m, makeWaterNormal(), {
      ...
      normalScale: 0.5,
      // 물가 정점은 smoothstep(1.6) 이 붙박아 두므로 한가운데 진폭은 더 여유가
      // 있다 — "출렁이는 게 안 보인다"는 지적에 0.045 → 0.055, 속도 0.6 → 0.9.
      // 물가 1유닛 안쪽 상승분은 여전히 둑 어깨(+0.04) 아래다.
      swellAmp: 0.055,
      swellSpeed: 0.9,
      foamWidth: 0.5,
      skyTint: "#bcdcea",
