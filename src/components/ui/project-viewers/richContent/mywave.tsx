  ...
  return (
    <div className="overflow-hidden rounded-xl border" style={{borderColor: `${theme.primary}25`, background: "#070d12"}}>
      <div className="flex items-center gap-2 border-b px-4 py-2" style={{borderColor: `${theme.primary}18`, background: "rgba(255,255,255,0.02)"}}>
        ...
        <span className="ml-2 font-mono text-[11px] font-bold text-white/45">{filename}</span>
      </div>
        ...
        <pre className="font-mono text-[12px] leading-[1.7]">
          {lines.map((line, i) => (
            <div
              key={i}
              className="flex"
              style={highlightLines.includes(i + 1) ? {background: `${theme.primary}12`, marginInline: -16, paddingInline: 16} : undefined}
            >
              <span className="mr-4 select-none text-right text-white/20" style={{minWidth: 18}}>
                {i + 1}
              </span>
      ...
      {caption ? (
        <div className="border-t px-4 py-2 font-mono text-[11px] text-white/40" style={{borderColor: `${theme.primary}15`}}>
          {"// "}
          {caption}
        </div>
      ) : null}
...
function ScreenFrame({title, theme, children}: {title: string; theme: ProjectTheme; children: React.ReactNode}) {
  return (
    <div className="overflow-hidden rounded-xl border" style={{borderColor: `${theme.primary}25`, background: "#06100c"}}>
      <div className="flex items-center gap-2 border-b px-3 py-2" style={{borderColor: `${theme.primary}18`, background: "rgba(255,255,255,0.02)"}}>
        ...
        <div className="ml-2 flex-1 rounded bg-white/5 px-2 py-0.5 font-mono text-[10px] text-white/35">{title}</div>
      </div>
      {children}
    </div>
  );
...
function MockScreen({variant, theme}: {variant: "dashboard" | "flow" | "goals" | "report"; theme: ProjectTheme}) {
  if (variant === "dashboard") {
    return (
      <ScreenFrame title="mywave.app/dashboard" theme={theme}>
        <div className="flex">
          <div className="hidden w-24 shrink-0 flex-col gap-1 border-r p-2.5 sm:flex" style={{borderColor: `${theme.primary}12`}}>
            <div className="mb-1.5 font-mono text-[11px] font-black" style={{color: theme.primary}}>◈ MyWave</div>
            {["대시보드", "흐름", "목표", "리포트"].map((m, i) => (
              <div key={m} className="rounded px-1.5 py-1 font-mono text-[9px] font-bold" style={i === 0 ? {background: `${theme.primary}1a`, color: theme.primary} : {color: "rgba(255,255,255,0.4)"}}>{m}</div>
            ))}
          </div>
          <div className="flex-1 p-3">
            <div className="mb-2 grid grid-cols-3 gap-1.5">
              {[{l: "총 자산", v: "₩12.4M"}, {l: "이번 달", v: "+8.2%"}, {l: "목표", v: "64%"}].map((k) => (
            ...
            <div className="rounded-lg border p-2.5" style={{borderColor: `${theme.primary}1a`}}>
              <p className="mb-1 font-mono text-[9px] font-bold text-white/45">이번 달 자산 흐름</p>
              <AreaSvg theme={theme} pts={[12, 18, 15, 26, 22, 34, 30, 42, 48, 44, 58, 64]} />
            </div>
          </div>
        </div>
      </ScreenFrame>
  ...
  if (variant === "flow") {
    return (
      <ScreenFrame title="mywave.app/flow" theme={theme}>
        <div className="p-3">
          <p className="mb-2 font-mono text-[10px] font-bold text-white/45">소비 카테고리 흐름</p>
          {[{l: "고정지출", p: 72, v: "₩1.8M"}, {l: "식비", p: 48, v: "₩620K"}, {l: "여가", p: 30, v: "₩340K"}, {l: "투자", p: 90, v: "₩2.1M"}].map((c, i) => (
            <div key={c.l} className="mb-2">
              <div className="flex justify-between font-mono text-[10px] text-white/55"><span>{c.l}</span><span>{c.v}</span></div>
              <div className="mt-1 h-1.5 overflow-hidden rounded-full bg-white/8">
                <motion.div className="h-full rounded-full" style={{background: theme.primary}} initial={{width: 0}} animate={{width: `${c.p}%`}} transition={{duration: 0.8, delay: i * 0.1}} />
              </div>
            </div>
          ))}
        </div>
    ...
    return (
      <ScreenFrame title="mywave.app/goals" theme={theme}>
        <div className="grid grid-cols-2 gap-2 p-3">
          {[{l: "비상금", p: 80, t: "₩5M"}, {l: "여행", p: 45, t: "₩3M"}, {l: "노트북", p: 100, t: "₩2M"}, {l: "투자 시드", p: 62, t: "₩10M"}].map((g, i) => (
            <div key={g.l} className="rounded-lg border p-2.5" style={{borderColor: `${theme.primary}1a`}}>
              <div className="flex items-center justify-between">
                <p className="font-mono text-[10px] font-bold text-white/70">{g.l}</p>
                <span className="font-mono text-[10px] font-black" style={{color: g.p >= 100 ? theme.primary : theme.accent}}>{g.p}%</span>
              </div>
              <p className="mt-0.5 font-mono text-[8px] text-white/35">목표 {g.t}</p>
              <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-white/8">
                <motion.div className="h-full rounded-full" style={{background: theme.primary}} initial={{width: 0}} animate={{width: `${g.p}%`}} transition={{duration: 0.8, delay: i * 0.08}} />
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <ScreenFrame title="mywave.app/report" theme={theme}>
      <div className="p-3">
        <p className="mb-2 font-mono text-[10px] font-bold text-white/45">월간 리포트 · 6월</p>
        <div className="mb-2 rounded-lg border p-2.5" style={{borderColor: `${theme.primary}1a`}}>
          <AreaSvg theme={theme} pts={[20, 28, 24, 38, 34, 30, 46, 52, 60, 66]} />
        </div>
        <div className="grid grid-cols-2 gap-1.5">
          {["저축률 28% ↑", "충동소비 31% ↓", "투자 비중 42%", "목표 3개 달성"].map((s) => (
            <div key={s} className="rounded border px-2 py-1.5 font-mono text-[9px] text-white/65" style={{borderColor: `${theme.primary}18`}}>{s}</div>
          ))}
        </div>
      </div>
    </ScreenFrame>
...
function ArchDiagram({theme}: {theme: ProjectTheme}) {
  const layers = [
    ...
    {name: "State", desc: "Zustand store · 거래·목표·필터 전역 상태", tag: "Store"},
    {name: "Domain Logic", desc: "useCashFlow · goalProgress · 통화 정규화", tag: "Hooks"},
      ...
      {layers.map((l, i) => (
        <div key={l.name}>
          <motion.div
            ...
            style={{borderColor: `${theme.primary}25`, background: `${theme.primary}08`}}
...
function DecisionTable({theme}: {theme: ProjectTheme}) {
  const rows = [
    {area: "차트", pick: "Recharts", why: "선언적 API로 빠른 구현, SVG 기반 커스터마이즈", alt: "visx(러닝커브 ↑), D3(과한 복잡도)"},
    {area: "상태관리", pick: "Zustand", why: "보일러플레이트 최소, 선택적 구독으로 리렌더 최소화", alt: "Redux(무거움), Context(전역 리렌더)"},
    {area: "스타일", pick: "Tailwind", why: "반응형을 빠르게, 디자인 일관성 유지", alt: "CSS Modules, styled-components"},
    {area: "데이터", pick: "API + localStorage", why: "MVP는 로컬로 빠르게, 이후 API 전환 용이하게 추상화", alt: "처음부터 풀백엔드(속도 ↓)"},
  ];
  return (
    <div className="overflow-hidden rounded-xl border" style={{borderColor: `${theme.primary}22`}}>
      <div className="grid grid-cols-[64px_88px_1fr] gap-2 border-b px-3 py-2 font-mono text-[10px] font-black uppercase tracking-wide text-white/35" style={{borderColor: `${theme.primary}18`, background: `${theme.primary}08`}}>
        <span>영역</span>
        <span>선택</span>
        <span>이유 / 고려한 대안</span>
      </div>
...
function ChallengeCard({
  ...
  problem: string;
  solution: string;
  code?: {filename: string; lines: string[]; caption?: string; highlightLines?: number[]};
}) {
  return (
    <motion.div
    ...
    >
      <div className="mb-2 flex items-center gap-2">
        <span className="flex h-6 w-6 items-center justify-center rounded font-mono text-xs font-black" style={{background: `${theme.primary}1a`, color: theme.primary}}>
          {String(index).padStart(2, "0")}
        </span>
        <p className="font-mono text-sm font-black text-white">{title}</p>
      </div>
      <div className="mb-1 flex gap-2">
        <span className="shrink-0 font-mono text-[11px] font-black text-[#f87171]">문제</span>
        <p className="text-[13px] leading-6 text-white/70">{problem}</p>
      </div>
      <div className="mb-3 flex gap-2">
        <span className="shrink-0 font-mono text-[11px] font-black" style={{color: theme.primary}}>해결</span>
        <p className="text-[13px] leading-6 text-white/80">{solution}</p>
      </div>
      {code ? <CodeBlock theme={theme} filename={code.filename} lines={code.lines} caption={code.caption} highlightLines={code.highlightLines} /> : null}
    </motion.div>
  );
}
...
function TldrBanner({theme}: {theme: ProjectTheme}) {
  const rows = [
    ...
    {k: "결과", v: "인터뷰 8명 중 대부분이 자신의 재정 상태를 즉시 이해"},
    {k: "내 역할", v: "기획 · UX 설계 · 프론트엔드 전체 (1인 프로젝트)"},
  ];
  return (
    <motion.div
      className="overflow-hidden rounded-xl border"
      style={{borderColor: `${theme.primary}40`, background: `${theme.primary}0a`}}
      initial={{opacity: 0, y: 12}}
      animate={{opacity: 1, y: 0}}
      transition={{duration: 0.45}}
    >
      <div className="flex items-center gap-2 border-b px-4 py-2" style={{borderColor: `${theme.primary}20`}}>
        <span className="font-mono text-[10px] font-black uppercase tracking-[0.25em]" style={{color: theme.primary}}>
          ★ TL;DR · 30초 요약
        </span>
      </div>
      <div className="p-4">
    ...
    <div className="grid gap-3 sm:grid-cols-[1.4fr_1fr]">
      ...
      >
        ...
        <motion.div
          ...
          animate={{boxShadow: [`0 0 0 0 ${theme.primary}55`, `0 0 0 16px ${theme.primary}00`]}}
          transition={{duration: 1.8, repeat: Infinity}}
        >
          <span className="ml-1 text-xl" style={{color: theme.bg}}>▶</span>
        </motion.div>
        <span className="absolute bottom-3 left-3 font-mono text-[11px] font-bold text-white/50">데모 영상 · 0:48</span>
      </motion.div>
      {/* 링크 버튼 */}
      <div className="flex flex-col justify-center gap-2.5">
        <a className="flex items-center justify-between rounded-xl border px-4 py-3 font-mono text-sm font-black transition hover:bg-white/5" style={{borderColor: `${theme.primary}40`, color: theme.primary}} href="#" onClick={(e) => e.preventDefault()}>
          🔗 라이브 데모 <span>↗</span>
        </a>
        <a className="flex items-center justify-between rounded-xl border px-4 py-3 font-mono text-sm font-black transition hover:bg-white/5" style={{borderColor: `${theme.primary}25`, color: theme.accent}} href="#" onClick={(e) => e.preventDefault()}>
...
function BeforeAfter({
  ...
  return (
    <div className="grid items-stretch gap-3 sm:grid-cols-[1fr_28px_1fr]">
      <div className="rounded-xl border p-4" style={{borderColor: "#f8717133", background: "#f871710a"}}>
        <p className="mb-2 font-mono text-[11px] font-black uppercase tracking-wide text-[#f87171]">Before · {before.label}</p>
        {before.body}
      </div>
      <div className="flex items-center justify-center font-mono text-lg font-black" style={{color: theme.primary}}>→</div>
      <div className="rounded-xl border p-4" style={{borderColor: `${theme.primary}40`, background: `${theme.primary}0a`}}>
        <p className="mb-2 font-mono text-[11px] font-black uppercase tracking-wide" style={{color: theme.primary}}>After · {after.label}</p>
        {after.body}
      </div>
    </div>
  );
        ...
        return (
          <div key={r.label}>
            <div className="mb-1.5 flex items-center justify-between">
              ...
              <span className="font-mono text-[11px] font-black" style={{color: theme.primary}}>
                {delta >= 0 ? "+" : ""}{delta}% {lowerBetter ? "↓" : "↑"}
              </span>
            </div>
            <div className="flex items-center gap-2">
              ...
              <div className="h-2 flex-1 overflow-hidden rounded-full bg-white/6">
                <motion.div className="h-full rounded-full bg-white/25" initial={{width: 0}} animate={{width: `${beforeW}%`}} transition={{duration: 0.7, delay: i * 0.1}} />
              </div>
            </div>
            <div className="mt-1 flex items-center gap-2">
              ...
              <div className="h-2 flex-1 overflow-hidden rounded-full bg-white/6">
                <motion.div className="h-full rounded-full" style={{background: theme.primary}} initial={{width: 0}} animate={{width: `${afterW}%`}} transition={{duration: 0.7, delay: 0.15 + i * 0.1}} />
              </div>
            </div>
          </div>
        );
      ...
      {steps.map((s, i) => (
        <div key={s.t} className="flex items-center">
          <motion.div
            ...
            transition={{delay: i * 0.08}}
          >
            <span className="font-mono text-[12px] font-black text-white">{s.t}</span>
...
export function MyWaveRichSection({
    ...
    return (
      <div className="flex flex-col gap-6 py-2">
        <div>
          ...
          <h1 className="mt-2 text-5xl font-black leading-tight text-white">{project.title}</h1>
          <p className="mt-3 max-w-2xl text-base leading-7 text-white/60">
            소비와 투자를 따로 보지 않고, 하나의 <span style={{color: theme.primary}}>‘돈의 흐름’</span>으로 읽는 개인 자산 관리 웹앱.
            거래 내역을 누적 흐름으로 시각화하고, 목표 대비 현재 위치를 한 장의 차트로 보여준다.
          </p>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="rounded-xl border p-4" style={{borderColor: `${theme.primary}22`}}>
            <MetaRow label="기간" value="2025.03 – 2025.06 · 4개월" theme={theme} />
            <MetaRow label="팀" value="개인 프로젝트 (기획·디자인·개발 1인)" theme={theme} />
            <MetaRow label="역할" value="서비스 기획 · UX 설계 · 프론트엔드 전체" theme={theme} />
            <MetaRow label="스택" value="React · TypeScript · Recharts · Zustand · Tailwind" theme={theme} />
            <MetaRow label="배포" value="Vercel · PWA 지원" theme={theme} />
          </div>
        ...
        <div>
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {[
              {t: "자산 흐름 시각화", d: "거래를 누적 area 차트로 — 흐름과 목표선을 한 화면에"},
      ...
      <div className="flex flex-col gap-6 py-2">
        <div>
          ...
          <p className="text-base leading-8 text-white/85">
            기존 가계부·금융 앱은 거래 내역을 <strong className="text-white">단순 표로 나열</strong>한다. 숫자는 많지만
            정작 “내 돈이 어디로 흘러가는지”, “목표까지 얼마나 왔는지”는 한눈에 보이지 않았다.
          </p>
        </div>

        <div>
          <SubLabel theme={theme}>사용자 리서치 · 인터뷰 8명</SubLabel>
          <div className="flex flex-col gap-2.5">
            <QuoteCard theme={theme} quote="가계부는 쓰는데, 막상 이번 달에 내가 잘 하고 있는 건지 모르겠어요." who="20대 직장인 A · 가계부 2년차" />
            <QuoteCard theme={theme} quote="투자랑 소비가 다른 앱에 흩어져 있어서 전체 그림이 안 그려져요." who="30대 프리랜서 B" />
            <div className="flex items-center gap-3 rounded-lg border px-4 py-3" style={{borderColor: `${theme.primary}22`}}>
              <span className="font-mono text-2xl font-black" style={{color: theme.primary}}>7/8</span>
              <span className="text-sm text-white/65">“내 소비·투자 흐름이 어떤지 한눈에 모르겠다”고 답함</span>
            </div>
          </div>
        </div>
            ...
            before={{
              ...
              body: (
                <div className="space-y-1">
                  {["06.21  스타벅스   -5,800", "06.20  급여     +2,400,000", "06.19  쿠팡     -32,100"].map((r) => (
                    <p key={r} className="font-mono text-[11px] text-white/45">{r}</p>
                  ))}
                  <p className="pt-1 font-mono text-[10px] text-[#f87171]">흐름·맥락 안 보임</p>
                </div>
              ),
            }}
            after={{
              ...
              body: (
                <div>
                  <AreaSvg theme={theme} pts={[10, 16, 14, 24, 20, 30, 28, 38, 44]} />
                  <p className="pt-1 font-mono text-[10px]" style={{color: theme.primary}}>한눈에 ‘지금 위치’가 보임</p>
                </div>
              ),
            }}
          />
        ...
        <div>
          ...
          <div className="rounded-xl border-l-2 p-4" style={{borderColor: theme.primary, background: `${theme.primary}08`}}>
            <p className="text-sm leading-7 text-white/80">
              “거래를 <strong className="text-white">누적 흐름 + 목표선</strong>으로 바꿔 보여주면, 사용자가 자신의 재정 상태를
              <strong className="text-white"> 즉시 직관적으로</strong> 이해할 것이다.”
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-6 py-2">
        ...
        <div>
          ...
          <p className="mb-3 text-sm leading-7 text-white/70">
            거래 ‘목록’을 <strong className="text-white">일별 누적 흐름</strong>으로 변환하고, 그 위에 목표선을 겹쳐
            “지금 어디쯤인지”를 한 장의 차트로 보여주도록 설계했다.
          </p>
    ...
    return (
      <div className="flex flex-col gap-6 py-2">
        <div>
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {[
              {g: "기획·UX", items: ["사용자 인터뷰 8명 진행·정리", "정보 구조(IA)와 화면 흐름 설계"]},
              {g: "프론트엔드", items: ["흐름 차트·목표·리포트 화면 구현", "반응형 + PWA 대응"]},
              ...
              <div key={group.g} className="rounded-lg border p-3" style={{borderColor: `${theme.primary}1a`}}>
                ...
                {group.items.map((it) => (
                  <div key={it} className="mb-1 flex items-start gap-2">
                    <span className="mt-0.5 font-mono text-xs" style={{color: theme.accent}}>✓</span>
                    <span className="text-[12px] leading-5 text-white/70">{it}</span>
                  </div>
                ))}
              </div>
            ))}
        ...
        <div>
          ...
          <div className="rounded-xl border p-4" style={{borderColor: `${theme.primary}22`}}>
            <CompareBars
              ...
              lowerBetter
              rows={[
                {label: "차트 리렌더 횟수 / 갱신", before: 47, after: 3, unit: "회"},
                {label: "프레임 시간", before: 120, after: 14, unit: "ms"},
                {label: "초기 번들 크기", before: 480, after: 210, unit: "KB"},
              ]}
            />
            <p className="mt-3 font-mono text-[11px] text-white/35">{"// "}React DevTools Profiler · Lighthouse 기준</p>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {["React", "TypeScript", "Recharts", "Zustand", "TailwindCSS", "Vite", "PWA"].map((t) => (
              <span key={t} className="rounded-lg border px-3 py-1.5 font-mono text-sm font-bold" style={{borderColor: `${theme.primary}35`, color: theme.accent}}>{t}</span>
            ))}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col gap-6 py-2">
      ...
      <div>
        <SubLabel theme={theme}>사용성 변화 · Before → After</SubLabel>
        <div className="rounded-xl border p-4" style={{borderColor: `${theme.primary}22`}}>
          <CompareBars
            theme={theme}
            lowerBetter={false}
            rows={[
              {label: "재정 상태 이해도 (5점 척도)", before: 2.1, after: 4.6, unit: "점"},
              {label: "월 저축률", before: 12, after: 28, unit: "%"},
              {label: "앱 재방문 (주간)", before: 1.4, after: 4.2, unit: "회"},
            ]}
          />
          <p className="mt-3 font-mono text-[11px] text-white/35">{"// "}사용자 8명 사전/사후 인터뷰 · 4주 사용</p>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          {[
            {k: "Keep", c: theme.primary, items: ["문제→가설→검증 사이클을 끝까지 돌린 것", "성능 이슈를 측정 기반으로 해결"]},
            {k: "Problem", c: "#f87171", items: ["초기 데이터 모델을 너무 일찍 고정", "테스트 커버리지 부족"]},
            {k: "Try", c: "#fbbf24", items: ["백엔드 연동해 실데이터 검증", "온보딩 A/B 테스트"]},
          ].map((col) => (
            <div key={col.k} className="rounded-xl border p-3" style={{borderColor: `${theme.primary}1a`}}>
              <p className="mb-2 font-mono text-[11px] font-black uppercase tracking-wide" style={{color: col.c}}>{col.k}</p>
              {col.items.map((it) => (
                <div key={it} className="mb-1.5 flex items-start gap-2">
                  <span className="mt-1 h-1 w-1 shrink-0 rounded-full" style={{background: col.c}} />
                  <span className="text-[12px] leading-5 text-white/65">{it}</span>
                </div>
              ))}
            </div>
          ))}
      ...
      <div>
        ...
        <div className="rounded-xl border-l-2 p-4" style={{borderColor: `${theme.primary}88`, background: `${theme.primary}08`}}>
          <p className="text-sm leading-7 text-white/75">
            데이터를 ‘보여주는 것’과 ‘읽히게 하는 것’은 다르다는 걸 체감했다. 같은 거래 데이터라도
            흐름·목표라는 맥락을 입히자 사용자의 반응이 완전히 달라졌고, 기술 선택은 ‘멋짐’이 아니라
            ‘이 문제에 맞는가’로 판단해야 한다는 걸 배웠다.
          </p>
        </div>
      </div>
