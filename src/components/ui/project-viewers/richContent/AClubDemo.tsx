...
const CLUBS: Club[] = [
  {name: "사운드웨이브", cat: "음악", emoji: "🎸", status: "모집중", members: 24},
  {name: "코드크래프트", cat: "개발", emoji: "💻", status: "모집중", members: 31},
    ...
    <DemoFrame
      ...
      footer={
        <p className="font-mono text-[11px] leading-5 text-white/45">
          💡 태그를 누르면 동아리 카드가 <span style={{color: theme.primary}}>실시간으로 필터·재배치</span>됩니다.
        </p>
      }
    >
      {/* 필터 칩 */}
        ...
        {CATS.map((c) => {
          ...
          return (
            <motion.button
              ...
              style={
                active
                  ? {borderColor: theme.primary, background: theme.primary, color: theme.bg}
                  : {borderColor: `${theme.primary}33`, color: theme.accent, background: "transparent"}
              }
            >
              {c}
            </motion.button>
        ...
        <AnimatePresence mode="popLayout">
          {list.map((club) => (
            ...
            >
              ...
              <span className="font-mono text-[12px] font-black text-white/85">{club.name}</span>
              <span className="font-mono text-[9px] text-white/35">{club.cat} · {club.members}명</span>
              <span
