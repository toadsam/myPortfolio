    ...
    <DemoFrame
      ...
      footer={
        <p className="font-mono text-[11px] leading-5 text-white/45">
          💡 운동을 기록할수록 <span style={{color: theme.primary}}>경험치가 쌓이고 레벨업</span> — 꾸준함을 게임처럼 보상합니다.
        </p>
      }
    >
      <div className="grid gap-4 lg:grid-cols-[1fr_1fr]">
        {/* 좌: 레벨 + XP 바 */}
        <div className="relative flex flex-col justify-center rounded-xl border p-5" style={{borderColor: `${theme.primary}22`, background: "rgba(0,0,0,0.2)"}}>
          <div className="flex items-end justify-between">
            <div className="flex items-center gap-3">
              <motion.div
                key={level}
                className="flex h-14 w-14 items-center justify-center rounded-2xl font-mono text-2xl font-black"
                style={{background: `${theme.primary}1a`, color: theme.primary, border: `1px solid ${theme.primary}55`}}
              ...
              <div>
                <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-white/40">LEVEL</p>
                <p className="font-mono text-sm font-black text-white">근력 Lv.{level}</p>
              </div>
            </div>
            <div className="text-right">
              <motion.p key={streak} className="font-mono text-lg font-black" style={{color: "#fb923c"}} initial={{scale: 1.4}} animate={{scale: 1}}>
                🔥 {streak}일
              </motion.p>
          ...
          <div className="mt-4">
            <div className="mb-1 flex justify-between font-mono text-[10px] text-white/45">
              <span>XP</span>
              <span>{into} / {PER_LEVEL}</span>
            </div>
            <div className="h-3 w-full overflow-hidden rounded-full" style={{background: "rgba(255,255,255,0.08)"}}>
              <motion.div className="h-full rounded-full" style={{background: `linear-gradient(90deg, ${theme.primary}, ${theme.accent})`}} animate={{width: `${pct}%`}} transition={{type: "spring", stiffness: 140, damping: 20}} />
            </div>
          </div>
          ...
          <AnimatePresence>
            {burst > 0 ? (
              ...
              >
                <motion.span className="font-mono text-2xl font-black" style={{color: theme.accent, textShadow: `0 0 24px ${theme.primary}`}} initial={{scale: 0.5}} animate={{scale: 1.15}}>
                  LEVEL UP! ⚡
                </motion.span>
              </motion.div>
            ) : null}
          ...
          <div className="grid grid-cols-2 gap-2">
            {WORKOUTS.map((w) => (
              <motion.button
                ...
                whileHover={{y: -3, borderColor: `${theme.primary}66`, background: `${theme.primary}10`}}
                whileTap={{scale: 0.95}}
              ...
              >
                ...
                <span className="font-mono text-[12px] font-bold text-white/85">{w.n}</span>
                <span className="font-mono text-[10px]" style={{color: theme.primary}}>+{w.xp} XP</span>
              </motion.button>
            ))}
          </div>
        </div>
