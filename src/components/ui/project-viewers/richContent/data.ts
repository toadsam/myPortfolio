...
export const RICH_DATA: Record<string, RichProject> = {
  ...
  aclub: {
    ...
    gallery: [
      ...
      // 게시 시각·인기 글·안내문은 게시글 캡처에, 섭외 규모는 답장 목록에.
      {
        src: "/projects/op/aclub-launch-post.webp",
        label: "출시 저녁 — 에브리타임 게시글 · 03/03 18:44",
        ratio: "1040/950",
        caption:
          "사람이 몰리는 저녁 6시대에 올렸다. 같은 날 밤 실시간 인기 글 2위. 아래 괄호가 그날 받은 문의의 답이다 — 에타 앱 안에서 열면 로그인 버튼이 사라져, 크롬·사파리로 열어 달라고 먼저 안내했다."
      },
      {
    ...
    challenges: [
      ...
      // 답장 수는 적지 않는다 — 화면에 보이는 것보다 많다고 주장할 근거가 없다.
      {
        ...
        solution:
          "동아리 회장 전원에게 직접 연락해 등록을 요청하고, 백엔드 팀원과 출시 일정을 맞췄다. 에브리타임 글은 사람이 몰리는 저녁 6시대에 올리고(03/03 18:44) 카톡·인스타로 같이 알렸다 — 그날 밤 실시간 인기 글에 올랐고, 회장들의 답장이 줄지어 왔다.\n\n문의도 직접 받았다. 에타 앱 안에서 링크를 열면 로그인 버튼이 사라지는 문제가 그날 나왔는데, 원인을 파는 대신 게시글에 「크롬·사파리로 열어 달라」 안내부터 붙였다. 출시 저녁엔 고치는 것보다 쓰게 하는 게 먼저였다. 그 링크로 들어온 유입이 28일 세션의 94%다."
      },
