...
export const RICH_DATA: Record<string, RichProject> = {
  ...
  muscleup: {
    ...
    decisions: [
      ...
      },
      {
        ...
        why: "동시 401 에서 서로의 토큰을 무효화해 로그아웃이 났다 — 서버 로테이션을 되돌리고 클라이언트에서 재발급을 한 번만(single-flight)",
    ...
    challenges: [
      ...
      },
      {
        ...
        solution:
          "정렬이 사라지고 인덱스를 거꾸로 훑는 계획으로 바뀐다. 목록 네 개가 668~2472배 빨라졌다. 다만 여기서 멈추면 틀린 결론이다 — Spring Data 의 Page 는 목록과 함께 count 를 날리는데 인덱스는 그걸 못 고친다. count 는 57.2ms 에서 57.4ms 로 그대로여서, 한 페이지 전체는 2.0배만 빨라진다. 이제 한 페이지의 99.9%가 count 다. 다음 병목은 정렬이 아니라 여기이고, 커서 페이지네이션이나 근사 카운트로 접근할 문제다. 인덱스 넷이 쓰는 디스크는 합쳐 21MB 였다.",
        code: {
      ...
      },
      {
        ...
        solution:
          "서버를 그대로 띄우고 동시 접속을 25 → 300 으로 올리며, 앱이 이미 가진 ping:check 로 왕복 시간을 쟀다. 50명까지는 p95 9ms — 회원 전원이 동시에 들어와도 여유가 크다. 다만 100명에서 p95 85ms 로 설계 주기 60ms 를 이미 넘는다. 재기 전에 내가 짐작한 한계는 이보다 훨씬 위였다.\n\n150에서 꺾이고 200부터 무너진다(p95 1.1초). 원인은 구조다 — 매 틱 전원에게 전원 목록을 보내니 바이트가 N² 로 큰다. 1인당 바이트는 795B 로 인원과 무관하게 고정인데, 서버 송신만 2.7 → 376MB/s 로 140배 뛴다(growthParams 숫자 19개가 대부분).\n\n무너지는 순간에도 CPU 는 18.9% 다 — 연산이 막힌 게 아니라 못 보내서 노는 것이다. 이동을 60ms 로 합친 건 잘한 결정이지만 다음 병목은 페이로드다. 뷰포트 컬링과 델타 전송이 먼저이고, 그 전에 서버 대수를 늘리면 대역폭은 그대로다.",
        code: {
  ...
  aclub: {
    ...
    gallery: [
      ...
      //
      // ── GA4 캡처 자리 (비어 있음) ─────────────────────────────────────
      // 여기 있던 `aclub-analytics.webp` 를 뺐다. **틀린 그림이라서가 아니라
      // 다른 기간이라서**다: 그 캡처는 2025 가 섞인 누계(조회수 6.7만 · 활성
      // 사용자 7.8천)인데, 위 지표 타일은 2026 단독(3,500 · 8.8만)이다.
      // 같은 이름의 지표가 두 값으로 나란히 놓이면 심사자는 숫자를 의심한다.
      // 크롭을 풀어 글자가 읽히게 되면서 그 모순이 오히려 드러났다.
      //
      // 2026 단독으로 가기로 했다 — 프로젝트장·프론트 리드로 맡은 기간이고,
      // 이력서 네 곳(hero·projects·resume·여기)이 이미 그 값으로 맞춰져 있다.
      //
      // 새 캡처를 받으면 아래를 그대로 살려 첫 줄 전폭으로 되돌린다.
      // 전폭인 이유는 둘이다 — 2.78:1 을 반 칸(594px)에 넣으면 글자가 0.6배로
      // 줄고, 3장이 2칸 격자에 들어가면 마지막 한 장이 둘째 줄에 혼자 남아
      // 오른쪽 절반이 통째로 빈다. 전폭이면 둘 다 사라진다.
      //
      //   {
      //     src: "/projects/op/aclub-analytics-2026.webp",
      //     label: "동아리 이용 분석 — GA4 · 2026.01–03",
      //     ratio: "<파일의 실제 가로/세로>",
      //     wide: true,
      //     caption: "…"
      //   },
      //
      // ⚠️ 캡처는 기간을 `2026.01.01–2026.03.15` 로 **직접 지정**해서 뜰 것.
      //    `올해(1월~오늘)` 로 두면 뜨는 날짜에 따라 누계가 계속 커져서,
      //    위 타일의 3,500 · 8.8만 과 다시 어긋난다.
      // ──────────────────────────────────────────────────────────────────
    ...
    // 기간을 밝히는 김에 **왜 그 기간인지**까지 적는다. 2025 를 합치면 활성
    // 사용자가 7.8천으로 커지지만, 그 해엔 프론트 3인 중 하나였다. 총괄로
    // 맡은 구간만 끊어 세는 편이 숫자는 작아도 주장은 정확하다.
    metricsNote:
      "Google Analytics 4 · 2026.01–03 · 활성 사용자 기준 — 프로젝트장·프론트 리드로 맡은 기간만 세었다(2025 팀원 시절 미합산).",
    kpt: {
      keep: [
        "데이터 요청을 훅으로 모은 것",
        "보호 라우트로 권한 일원화",
    ...
    learningLead:
      "‘같은 데이터를 어떻게 한 번만 정의해 재사용할까’가 팀 프론트엔드의 협업 속도를 좌우했다.",
    learning: "커스텀 훅과 보호 라우트가 그 답이었다."
  },
