...
import {CATEGORY_COLORS, CATEGORY_NAMES, CLUBS, SORT_LABELS, type CategoryName, type SortKey} from "../data";
...
function Motif({cat, color}: {cat: CategoryName; color: string}) {
  const common = {className: "h-10 w-10 opacity-[0.35]", viewBox: "0 0 24 24", fill: "none", stroke: color, strokeWidth: 1.5} as const;
  if (cat === "예술")
    return (
  ...
  if (cat === "봉사")
    return (
      <svg {...common} aria-hidden="true">
        <path d="M12 21a9 9 0 1 1 0-18 9 9 0 0 1 0 18z" fill={color} fillOpacity="0.1" />
...
function LiveCount({count, reducedMotion}: {count: number; reducedMotion: boolean}) {
  ...
  return <span className="font-mono text-[14px] tabular-nums text-[#c084fc]">{display} / 24</span>;
}
...
const CodePanel = forwardRef<CodePanelHandle>(function CodePanel(_props, ref) {
  ...
  useImperativeHandle(ref, () => ({
    playHighlight() {
      ...
      step(2, 280, "// 정렬 기준이 같을 때 순서가 흔들리면 벽이 이유 없이 다시 섞인다");
      step(3, 420, "// 필터 객체가 그대로면 다시 계산하지 않는다");
      timers.current.push(window.setTimeout(() => setCaption(DEFAULT_CAPTION), 1620));
    },
  }));

  const Block = ({index, children}: {index: number; children: ReactNode}) => (
    <div ref={(el) => void (blocks.current[index] = el)} className="ac-sweep-target -ml-2 px-2 py-0.5">
      {children}
    </div>
  );
  ...
  return (
    <div className="relative flex h-full flex-col overflow-hidden rounded-md border border-[rgba(192,132,252,0.18)] bg-[#0f0a1a] shadow-[0_4px_16px_rgba(0,0,0,0.3)]">
      <div className="flex h-[36px] shrink-0 items-center border-b border-[rgba(192,132,252,0.18)] bg-[#170f26] px-4">
        ...
        <span className="font-mono text-[11px] text-[rgba(255,255,255,0.45)]">useClubFilter.ts</span>
      </div>
      ...
      <div className="ac-scroll-thin relative flex-1 overflow-auto pb-[56px] pt-4 font-mono text-[11px] leading-relaxed sm:text-[12px]">
        ...
        <div className="w-max min-w-full whitespace-pre pl-12 pr-4 text-[rgba(255,255,255,0.88)]">
          <Block index={0}>
            ...
            <Kw>boolean</Kw>
            {";\n};\n\n"}
            <Kw>type</Kw> ClubItem = {"{"} id: <Kw>string</Kw>; name: <Kw>string</Kw> {"}"};
          </Block>
      ...
      <div className="absolute inset-x-0 bottom-0 z-20 flex h-[36px] items-center border-t border-[rgba(192,132,252,0.12)] bg-[#0f0a1a] px-4 shadow-[0_-4px_12px_rgba(0,0,0,0.2)]">
        <div key={caption} className="font-mono text-[11px] text-[rgba(255,255,255,0.45)] transition-opacity duration-200">
          {caption}
        </div>
      </div>
    </div>
...
export function FilterSection() {
  ...
  useEffect(() => {
    ...
    if (!el) return;
    const io = new IntersectionObserver(([entry]) => setWallInView(Boolean(entry?.isIntersecting)), {
  ...
  const filtered = useMemo(() => {
    const list = CLUBS.filter((c) => {
      if (!filters.categories.includes("전체") && !filters.categories.includes(c.category)) return false;
    ...
    list.sort((a, b) => {
      if (filters.sort === "deadline") {
        ...
        if (a.recruiting && a.daysLeft !== b.daysLeft) return a.daysLeft - b.daysLeft;
  ...
  function update(partial: Partial<Filters> | ((prev: Filters) => Partial<Filters>)) {
    setFilters((prev) => {
      const next = typeof partial === "function" ? {...prev, ...partial(prev)} : {...prev, ...partial};
      setHistory((h) => [...h, prev]);
  ...
  const tapeVariants = {hidden: {scaleX: 0}, visible: {scaleX: 1, transition: {duration: 0.2}}};
      ...
      <div className="relative mx-auto w-full max-w-[1180px] px-6">
        <div className="max-w-[740px]">
          ...
          <h2 className="mt-3 flex flex-wrap gap-x-2 text-[28px] font-black text-[rgba(255,255,255,0.88)]">
            {"검색창에 뭘 쳐야 할지 모르는 사람을 위한 화면".split(" ").map((w, i) => (
              <motion.span
                key={i}
                className="inline-block"
                initial={{opacity: 0, y: 10}}
                animate={phase >= 0 ? {opacity: 1, y: 0} : {opacity: 0, y: 10}}
          ...
          >
            ...
            <br className="hidden sm:block" />
            아래 필터를 <strong className="font-bold text-[#d8b4fe]">직접 눌러보세요</strong>. 오른쪽 코드에서 지금 어느
            단계가 도는지 보입니다.
          </motion.p>
        </div>
          ...
          <div className="flex flex-col items-start justify-between gap-4 md:flex-row md:gap-0">
            <div className="flex w-full flex-1 flex-col gap-3 overflow-hidden">
              <div className="ac-no-scrollbar flex w-full items-center gap-2 overflow-x-auto whitespace-nowrap pb-1 md:pb-0">
                <span className="mr-2 shrink-0 font-mono text-[10px] text-[rgba(255,255,255,0.46)]">분야</span>
                {chips.map((cat, i) => {
                  const isActive = filters.categories.includes(cat);
                  const activeColor = cat === "전체" ? "#c084fc" : CATEGORY_COLORS[cat];
                  ...
                  return (
                    <motion.button
                      ...
                      style={{
                        border: `1px solid ${isActive ? activeColor : "rgba(255,255,255,0.14)"}`,
                        backgroundColor: isActive ? `${activeColor}24` : "transparent",
                ...
                {!hasInteracted && phase >= 6 ? (
                  <span className="ml-2 font-mono text-[10px] text-[rgba(255,255,255,0.35)]">조건을 눌러보세요</span>
                ) : null}
              </div>

              <div className="ac-no-scrollbar flex w-full items-center gap-4 overflow-x-auto whitespace-nowrap pb-1 md:pb-0">
                <span className="mr-2 shrink-0 font-mono text-[10px] text-[rgba(255,255,255,0.46)]">조건</span>
                <Toggle label="모집중만" checked={filters.recruitingOnly} onChange={() => update((p) => ({recruitingOnly: !p.recruitingOnly}))} />
                <Toggle label="신입생 환영" checked={filters.newbieWelcome} onChange={() => update((p) => ({newbieWelcome: !p.newbieWelcome}))} />
                <Toggle label="주말 활동 없음" checked={filters.noWeekend} onChange={() => update((p) => ({noWeekend: !p.noWeekend}))} />
              </div>

              <div className="mt-1 flex items-center gap-2 pb-1 md:pb-0">
                <span className="mr-2 shrink-0 font-mono text-[10px] text-[rgba(255,255,255,0.46)]">정렬</span>
                <div
                  role="radiogroup"
                  aria-label="정렬"
            ...
            <div className="flex w-full shrink-0 flex-row items-center justify-between border-t border-[rgba(192,132,252,0.10)] pt-3 md:w-auto md:flex-col md:items-end md:border-t-0 md:pt-0">
              <LiveCount count={filtered.length} reducedMotion={reducedMotion} />
              <button
                type="button"
                onClick={resetAll}
        ...
        {/* ── 포스터 벽 + 코드 ── */}
        <div ref={wallRef} className="mt-10 flex flex-col gap-[18px] min-[1100px]:flex-row">
          <div className="min-h-[560px] w-full min-[1100px]:w-[62%]">
            <ul role="list" className="grid grid-cols-2 gap-[14px] sm:grid-cols-3 min-[1100px]:grid-cols-4">
              <AnimatePresence mode="popLayout">
                    ...
                    return (
                      <motion.li
                        ...
                        animate={{
                          ...
                          scale: 1,
                          transition: {
                            delay: animate ? (initialMount ? i * 0.025 : 0.3 + i * 0.03) : 0,
                        ...
                        exit={{
                          ...
                          y: 120,
                          rotate: c.exitRotate,
                          transition: {duration: animate ? 0.42 : 0, ease: [0.4, 0, 1, 1], delay: animate ? i * 0.015 : 0},
                        }}
                        transition={{layout: {duration: animate ? 0.5 : 0, ease: EASE}}}
                      ...
                      >
                        ...
                        <div className="relative mb-3 flex h-[62px] w-full items-center justify-center overflow-hidden rounded-sm border border-[rgba(255,255,255,0.04)] bg-[rgba(13,8,22,0.5)]">
                          <Motif cat={c.category} color={CATEGORY_COLORS[c.category]} />
                        </div>
                        ...
                        />
                        {c.daysLeft >= 0 ? (
                          <div className={`absolute bottom-3 left-3 font-mono text-[9px] tabular-nums ${deadlineColor}`}>
                            D-{c.daysLeft}
                          </div>
                        ) : null}
                        <div
                ...
                {phase >= 4 && filtered.length === 0 ? (
                  <motion.li
                    ...
                    animate={{opacity: 1, scale: 1, transition: {duration: 0.3, delay: 0.4}}}
                  ...
                  >
                    <div className="font-mono text-[13px] text-[rgba(255,255,255,0.55)]">조건에 맞는 동아리가 없습니다</div>
                    <div className="mb-4 mt-1 font-mono text-[11px] text-[rgba(255,255,255,0.40)]">조건을 하나 풀어보세요</div>
                    <button
                      type="button"
                      onClick={releaseLast}
        ...
        >
          ...
          >
            <div className="mb-3 font-mono text-[10px] tracking-[0.2em] text-[#c084fc]">왜 애니메이션을 넣었나</div>
            <div className="text-[15px] leading-[32px] text-[rgba(255,255,255,0.88)]">
              ...
              <br />
              그래서 <strong className="font-bold text-[#d8b4fe]">사라지는 것과 남는 것의 움직임을 일부러 다르게</strong>{" "}
              만들었다.
            </div>
          </div>
        </motion.div>
