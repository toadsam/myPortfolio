...
import {CodeWindow, HlLine, Kicker, Kw, Reveal, RevealWords, Str, useInViewOnce} from "../parts";

type View = "assembled" | "exploded";

const LAYERS = [
  {no: 1, name: "CategoryBar", reuse: "4개 화면에서 사용", leader: "w-[20px] sm:w-[40px]"},
  {no: 2, name: "CoverImage", reuse: "3개 화면에서 사용", leader: "w-[30px] sm:w-[50px]"},
  {no: 3, name: "ClubHeading", reuse: "4개 화면에서 사용", leader: "w-[20px] sm:w-[40px]"},
  {no: 4, name: "TagRow", reuse: "4개 화면에서 사용", leader: "w-[30px] sm:w-[50px]"},
  {no: 5, name: "StatusBar", reuse: "2개 화면에서 사용", leader: "w-[20px] sm:w-[40px]"},
];
...
const TABLE = [
  {name: "CategoryBar", takes: "분야 값", rejects: "동아리 전체 객체", used: "목록 · 상세 · 마이 · 관리자"},
  {name: "CoverImage", takes: "이미지 주소 · 대체 텍스트", rejects: "로딩 상태", used: "목록 · 상세 · 마이"},
  {name: "ClubHeading", takes: "이름 · 한 줄 소개 · 크기", rejects: "클릭 동작", used: "목록 · 상세 · 마이 · 관리자"},
  {name: "TagRow", takes: "태그 배열", rejects: "필터 상태", used: "목록 · 상세 · 마이 · 관리자"},
  {name: "StatusBar", takes: "모집 여부 · 마감일", rejects: "지원 로직", used: "목록 · 상세"},
];

export function ComponentSection() {
  const {reducedMotion, announce} = useAClub();
  const {ref: sectionRef, inView} = useInViewOnce<HTMLElement>({threshold: 0.1});
  const {ref: stageRef, inView: stageInView} = useInViewOnce<HTMLDivElement>({threshold: 0.5});
  ...
  useEffect(() => {
    if (!inView) return;
    const t = window.setTimeout(() => setStripVisible(true), 1200 + VARIANTS.length * 80);
  ...
  function setState(next: View, byUser = true) {
    ...
    announce(
      next === "exploded" ? "컴포넌트가 분해되었습니다. 5개의 계층 구조를 보여줍니다." : "컴포넌트가 다시 조립되었습니다.",
    );
    ...
    if (next === "exploded") {
      ...
      LAYERS.forEach((_, i) => {
        timers.current.push(window.setTimeout(() => setAnnotations((n) => Math.max(n, i + 1)), 400 + i * 90));
      });
  ...
  function layerVisual(no: number) {
    ...
    return {
      opacity: dim ? 0.35 : 1,
      filter: dim ? "grayscale(0.5)" : highlight?.includes(no) ? "brightness(1.1)" : "none",
    ...
    >
      <div className="mx-auto flex max-w-[1080px] flex-col items-center px-6 py-[120px]">
        <div className="w-full">
          ...
          <Reveal delay={0.7} className="mt-5 max-w-[740px]">
            <p className="text-[16px] leading-8 text-[rgba(255,255,255,0.88)]">
              목록에서는 작게, 상세에서는 크게, 마이페이지에서는 상태 배지가 붙고, 관리자 화면에서는 아예 가로로 눕는다.
        ...
        >
          <div className="absolute right-5 top-5 z-50 flex flex-col items-end gap-2 sm:right-7 sm:top-7">
            ...
            >
              <div
                ...
                style={{left: exploded ? "50%" : "4px", width: "calc(50% - 4px)", transitionTimingFunction: "cubic-bezier(0.22,1,0.36,1)"}}
              />
              {(["assembled", "exploded"] as View[]).map((v, i) => (
                <button
                  ...
                  style={{color: view === v ? "#c084fc" : "rgba(255,255,255,0.46)"}}
                >
                  {v === "assembled" ? "조립" : "분해"}
                </button>
              ))}
          ...
          >
            <div className="relative flex w-[280px] flex-col rounded-md border border-white/5 bg-[#0f0a1a]" style={{transformStyle: "preserve-3d"}}>
              {LAYERS.map((layer, i) => {
                ...
                return (
                  ...
                  >
                    {layer.no === 1 ? <div className="h-1 w-full rounded-t-md bg-[#f472b6]" /> : null}

                    {layer.no === 2 ? (
                      <div className="flex h-[130px] items-center justify-center border-y border-white/5 bg-[#170f26]">
                        <span className="select-none font-mono text-[11px] text-white/30">[IMG-SLOT]</span>
                      </div>
                    ) : null}

                    {layer.no === 3 ? (
                      <div className="bg-[#0f0a1a] p-4 pb-2">
                        <h3 className="text-[18px] font-black leading-tight text-[rgba(255,255,255,0.88)]">빛그림 사진회</h3>
                    ...
                    <div
                      ...
                      style={{opacity: annotationOn ? 1 : 0, transform: `translateY(-50%) translateX(${annotationOn ? 0 : 32}px)`}}
                    >
                      <div
                        className={`${layer.leader} mr-2 h-px origin-left bg-[#c084fc]/40 transition-transform duration-300 sm:mr-3`}
                        style={{transform: annotationOn ? "scaleX(1)" : "scaleX(0)"}}
                      />
                      <div className="flex flex-col">
                        <span
                          ...
                          style={{opacity: annotationOn ? 1 : 0, transform: annotationOn ? "none" : "translateX(8px)"}}
                        >
                          {layer.name}
                        </span>
                        <span
          ...
          <div
            ...
            style={{opacity: chipVisible ? 1 : 0, transform: chipVisible ? "none" : "translateY(8px)"}}
          >
            컴포넌트 5종 · 사용처 17곳
          </div>
        ...
        <Reveal className="mt-[48px] w-full">
          ...
          <div className="hidden overflow-hidden rounded-md border border-white/10 bg-[#0f0a1a] shadow-lg sm:grid sm:grid-cols-[1.2fr_1.5fr_1.5fr_1fr]">
            ...
            {TABLE.map((row, r) => {
              ...
              return [
                <div key={`${row.name}-n`} className={`px-4 py-3 font-mono text-[12px] text-white/80 ${edge}`}>
                  {row.name}
                </div>,
                <div key={`${row.name}-t`} className={`border-l border-white/10 px-4 py-3 font-mono text-[12px] text-white/70 ${edge}`}>
                  {row.takes}
                </div>,
                <div
                  key={`${row.name}-r`}
                ...
                <div key={`${row.name}-u`} className={`border-l border-white/10 px-4 py-3 font-mono text-[12px] text-[#c084fc] ${edge}`}>
                  {row.used}
                </div>,
              ];
            })}
          ...
          <div className="flex flex-col gap-3 sm:hidden">
            {TABLE.map((row) => (
              <div key={row.name} className="rounded-md border border-white/10 bg-[#0f0a1a] p-4">
                <div className="mb-2 font-mono text-[12px] font-bold text-[#c084fc]">{row.name}</div>
                <div className="font-mono text-[11px] text-white/70">받는 것: {row.takes}</div>
                <div className="mt-1 font-mono text-[11px] text-[rgba(255,255,255,0.55)]">✕ 안 받는 것: {row.rejects}</div>
                <div className="mt-2 font-mono text-[11px] text-[#c084fc]">사용처: {row.used}</div>
              </div>
            ))}
          </div>
          ...
          <p className="mt-6 max-w-[800px] text-[15px] leading-8 text-[rgba(255,255,255,0.88)]">
            ...
            <strong className="font-bold text-[#d8b4fe]">그 값이 어디서 왔는지는 모른다</strong>. StatusBar가 지원 로직을
            알기 시작하면 관리자 화면에서는 못 쓴다.
          </p>
        </Reveal>
        ...
        <Reveal className="mt-[40px] w-full">
          <CodeWindow
            file="ClubCard.tsx"
            footer={"// 카드가 라우터를 알기 시작하면, 라우터가 없는 화면에서는 못 쓴다"}
            className="shadow-xl"
          >
            <HlLine>
              <Kw>type</Kw> ClubCardVm = {"{"} name: <Kw>string</Kw>; intro: <Kw>string</Kw>; coverUrl: <Kw>string</Kw>;
              category: <Kw>string</Kw>; tags: <Kw>string</Kw>[]; recruiting: <Kw>boolean</Kw>; deadline: <Kw>string</Kw>;{" "}
              {"}"};
            </HlLine>
            <HlLine>
              <Kw>type</Kw> Variant = <Str>&quot;list&quot;</Str> | <Str>&quot;detail&quot;</Str> |{" "}
              <Str>&quot;mypage&quot;</Str> | <Str>&quot;admin&quot;</Str>;
            </HlLine>
            <Kw>interface</Kw>
            {" Props { vm: ClubCardVm; variant: Variant; onSelect?: () => "}
            <Kw>void</Kw>
            {"; }\n\n"}
            <span style={{color: "#7a5f8a"}}>{"// 카드가 라우터를 알기 시작하면, 라우터가 없는 화면에서는 못 쓴다"}</span>
            {"\n"}
        ...
        <Reveal className="mt-6 w-full max-w-[800px]">
          <p className="text-[15px] leading-8 text-[rgba(255,255,255,0.88)]">
            variant를 4개까지 늘린 건 사실 <strong className="font-bold text-[#fbbf24]">아슬아슬한 선택이었다</strong>.
            여기서 하나만 더 늘었으면 컴포넌트를 쪼개야 했을 것이다. 지금 구조는 「4개까지는 괜찮다」는 판단이지, 무한히
            늘려도 된다는 뜻은 아니다.
          </p>
        </Reveal>
      </div>
