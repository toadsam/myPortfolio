...
const COUNTS: Record<Role, string> = {
  student: "표시 항목 8",
  admin: "표시 항목 17"
};
...
export function RoleSection() {
  ...
  const {ref: cardRef, inView: cardInView} = useInViewOnce<HTMLDivElement>({
    threshold: 0.5
  });
    ...
    >
      <div className="mx-auto flex max-w-[1080px] flex-col items-center px-6 pb-[120px]">
        <div className="mb-6 w-full">
          ...
          >
            같은 동아리 데이터인데 보는 사람에 따라 필요한 게 완전히 다르다.
            학생은 「어떤 동아리인지」를 보고, 운영진은 「몇 명이 지원했는지」를
            본다. 두 화면을{" "}
          ...
          >
            ...
            {(["student", "admin"] as Role[]).map((r, i) => (
              <button
                ...
                style={{
                  color: role === r ? "#c084fc" : "rgba(255,255,255,0.46)"
                }}
              >
                {r === "student" ? "학생" : "운영진"}
              </button>
        ...
        <Reveal className="mt-10 w-full max-w-[700px]">
          <div className="mb-2 flex justify-end">
            <span className="font-mono text-[10px] text-[rgba(255,255,255,0.46)]">
              {COUNTS[role]}
            </span>
          </div>
          <div
              ...
              >
                <div className="flex w-[260px] flex-col overflow-hidden rounded-md border border-white/5 bg-[#0f0a1a]">
                  ...
                  <div className="flex flex-col gap-3 p-4">
                    <h3 className="text-[18px] font-black text-[rgba(255,255,255,0.88)]">
                      빛그림 사진회
                    </h3>
                    <span className="w-fit font-mono text-[10px] text-[#f472b6]">
                      예술
                    </span>
                ...
                <div className="flex h-full w-full flex-col gap-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <h3 className="text-[15px] font-black text-[rgba(255,255,255,0.88)]">
                        빛그림 사진회
                      </h3>
                  ...
                  <div className="grid grid-cols-4 gap-3">
                    {[
                      ...
                      <div
                        key={s.l}
                        className="rounded border border-white/5 bg-[#0f0a1a] p-3"
                      >
                        <div
                          className="font-mono text-[20px] font-black tabular-nums"
                          style={{color: s.c}}
                        >
                          {s.v}
                        </div>
                        <div className="mt-1 font-mono text-[9px] text-[rgba(255,255,255,0.46)]">
                          {s.l}
                        </div>
                      </div>
                    ))}
                      ...
                      {APPLICANTS.map((a, i) => (
                        ...
                        >
                          ...
                          <span className="flex items-center gap-2">
                            <span className="font-mono text-[9px] text-[rgba(255,255,255,0.46)]">
                              {a.date}
                            </span>
                            <span
              ...
              <div className="flex h-[calc(100%-26px)] flex-col gap-3 p-4">
                ...
                <div className="grid flex-1 grid-cols-3 gap-2">
                  {Array.from({length: 6}, (_, i) => (
                    <div
                      key={i}
                      className="flex flex-col gap-2 rounded border border-white/5 bg-white/5 p-2"
                    >
                ...
                <div className="flex flex-1 flex-col gap-3">
                  <div className="grid grid-cols-4 gap-2">
                    {[0, 1, 2, 3].map(i => (
                      <div
                        key={i}
                        className="h-14 rounded border border-white/5 bg-[#0f0a1a]"
                      />
                    ))}
                  </div>
          ...
          <div className="mt-8 w-full">
            <div className="hidden grid-cols-3 overflow-hidden rounded-md border border-[rgba(255,255,255,0.08)] sm:grid">
              ...
              {COMPARISON.map((row, r) => {
                ...
                return [
                  <div
                    key={`${row.item}-i`}
                    className={`px-4 py-[13px] font-mono text-[12px] text-white/70 ${
            ...
            <div className="flex flex-col gap-3 sm:hidden">
              {COMPARISON.map(row => (
                <div
                  key={row.item}
                  className="rounded-md border border-[rgba(255,255,255,0.08)] bg-[#0f0a1a] p-4"
                >
                  <div className="mb-2 font-mono text-[12px] text-[#c084fc]">
                    {row.item}
                  </div>
        ...
        <Reveal className="mt-11 w-full">
          <NoteBox label="분명히 해둘 것" tone="warn">
            <p className="text-[15px] leading-8 text-[rgba(255,255,255,0.88)]">
              ...
              백엔드가 없으니 프론트에서 역할에 따라 다른 화면을 그리는 데까지만
              했다. 실제로는 서버가 요청마다 권한을 확인해야 하고, 프론트의
              분기는 그걸 대신할 수 없다. 이 차이를 모르고 만든 게 아니라,
              알면서 여기서 멈춘 것이다.
            </p>
          </NoteBox>
        </Reveal>
        ...
        <Reveal className="mt-8 w-full">
          ...
          <div className="mt-2 flex flex-col gap-[14px] sm:flex-row">
            <p className="flex-1 px-3 py-2 font-mono text-[11px] text-[rgba(255,255,255,0.48)]">
              실제 학생 화면
            </p>
            <p className="flex-1 px-3 py-2 font-mono text-[11px] text-[rgba(255,255,255,0.48)]">
              실제 관리자 화면
