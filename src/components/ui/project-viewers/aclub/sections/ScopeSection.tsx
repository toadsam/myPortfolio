...
const DONE: {label: string; rotFinal: string; rotOver: string; delay: string; glyph: ReactNode}[] = [
  {
    label: "동아리 탐색 화면",
    rotFinal: "-1.5deg",
    rotOver: "-3deg",
  ...
  },
  {
    ...
    glyph: (
      <>
        ...
        <svg className="h-3 w-3 text-white/30" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
...
const NOT_DONE = [
  ...
  {label: "로그인 · 인증", status: "화면만 있음", tip: "역할 전환은 프론트에서만 이뤄집니다"},
  {label: "데이터베이스", status: "미구현", tip: "새로고침하면 상태가 초기화됩니다"},
  {label: "실제 배포", status: "안 함", tip: "로컬에서만 실행해봤습니다"},
  {label: "실사용자 테스트", status: "안 함", tip: "팀 내부에서만 확인했습니다"},
...
export function ScopeSection() {
  ...
  useEffect(() => {
    ...
    push(() => {
      setWallActive(true);
      DONE.forEach((p, i) => {
        push(() => setDropped((s) => new Set(s).add(i)), parseFloat(p.delay) * 1000 + 650);
      });
    }, 1600);
    push(() => {
      setDividerOn(true);
    ...
    push(() => {
      NOT_DONE.forEach((_, i) => push(() => setEmptyIn((n) => Math.max(n, i + 1)), i * 70));
    }, 2900);
      ...
      <div className="mx-auto flex max-w-[1040px] flex-col items-center px-6 py-[120px]">
        <div className="w-full">
          ...
          <p
            ...
            style={{opacity: phase >= 2 ? 1 : 0, transform: phase >= 2 ? "none" : "translateY(16px)"}}
          >
            포트폴리오에서는 프로젝트를 크게 보이게 만들 방법이 많습니다. 그런데 면접에서 한 번만 물어보면 다 드러납니다.
        ...
        <div
          ...
          style={{opacity: phase >= 3 ? 1 : 0, transform: phase >= 3 ? "none" : "translateY(32px)"}}
        >
          <div className="ac-noise" aria-hidden="true" />

          <div className="sr-only">
            이 섹션은 프로젝트의 구현 범위와 미구현 범위를 보여줍니다. 완료된 항목 6개:{" "}
            {DONE.map((d) => d.label).join(", ")}. 미구현 또는 축소된 항목 6개: {NOT_DONE.map((d) => d.label).join(", ")}.
          </div>
          ...
          <div className="relative z-10 grid h-full w-full grid-cols-2 place-items-center gap-[16px] sm:grid-cols-3 min-[900px]:grid-cols-6 min-[900px]:place-items-stretch">
            {DONE.map((p, i) => (
              ...
              >
                ...
                <div className="flex flex-1 flex-col items-center justify-center gap-1.5 p-3 opacity-40">{p.glyph}</div>
                <div className="relative mt-auto border-t border-[rgba(255,255,255,0.06)] bg-[#130d1c] p-3">
                  ...
                  <div className="absolute bottom-2 right-2 font-mono text-[9px] tabular-nums text-[#4ade80]">완료</div>
                </div>
            ...
            {NOT_DONE.map((p, i) => (
              ...
              >
                {[
                  ...
                  <span key={pos} className={`absolute ${pos} h-[6px] w-[6px] rounded-[1px] bg-[rgba(255,255,255,0.05)]`} />
                ))}

                <div className="mb-1 font-mono text-[11px] text-[rgba(255,255,255,0.42)]">{p.label}</div>
                <div className="font-mono text-[9px] text-[rgba(255,255,255,0.30)]">{p.status}</div>
                ...
                <div
                  ...
                  className={`pointer-events-none absolute bottom-full z-30 mb-3 whitespace-nowrap rounded border border-[rgba(255,255,255,0.16)] bg-[#0f0a1a] px-[10px] py-[6px] font-mono text-[10px] text-[rgba(255,255,255,0.88)] opacity-0 shadow-xl transition-opacity duration-200 group-hover:opacity-100 group-focus:opacity-100 ${
                    i === NOT_DONE.length - 1 ? "min-[900px]:right-0 min-[900px]:translate-x-4" : ""
                  }`}
                >
                  {p.tip}
                </div>
        ...
        <Reveal className="mt-[44px] w-full rounded-md border border-[rgba(192,132,252,0.24)] border-l-[3px] border-l-[#c084fc] bg-[rgba(192,132,252,0.04)] p-[24px]">
          <div className="mb-4 font-mono text-[11px] tracking-[0.2em] text-[#c084fc]">왜 여기서 멈췄나</div>
          <p className="mb-4 text-[16px] leading-[36px] text-[rgba(255,255,255,0.88)]">
            이 프로젝트에서 제가 맡은 건 프론트엔드였고, 팀에서 백엔드는 별도로 진행 중이었습니다. API가 나오기를 기다리는
            대신 mock data로 화면 흐름을 전부 완성해뒀습니다.
          </p>
          <p className="text-[16px] leading-[36px] text-[rgba(255,255,255,0.88)]">
            덕분에 탐색부터 지원까지의 흐름과 필터·라우팅 문제를 먼저 다 겪었습니다.{" "}
            <strong className="font-bold text-[#d8b4fe]">데이터 출처만 바꾸면 되도록</strong> 만들어두는 것까지가 이번
            범위였습니다.
          </p>
        </Reveal>
        ...
        <Reveal className="mt-[36px] flex w-full flex-col gap-[16px] md:flex-row">
          <div className="flex-1 rounded-md border border-[rgba(192,132,252,0.22)] bg-[rgba(192,132,252,0.04)] p-[20px]">
            <div className="mb-5 font-mono text-[10px] tracking-[0.18em] text-[#c084fc]">제가 한 일</div>
            <ul className="space-y-2">
              {[
                ...
                <li key={t} className="flex items-start gap-2 text-[15px] leading-[32px] text-[rgba(255,255,255,0.88)]">
          ...
          <div className="flex-1 rounded-md border border-[rgba(255,255,255,0.14)] bg-[rgba(255,255,255,0.02)] p-[20px]">
            ...
            <ul className="space-y-2">
              {[
                {mark: "○", text: "긴 사용자 흐름을 화면 단위로 설계할 수 있다", dim: false},
                {mark: "○", text: "브라우저 기본 동작(주소·뒤로가기·스크롤)을 다룰 수 있다", dim: false},
                {mark: "✕", text: "서버 구현과 실제 권한 처리는 이 프로젝트로 보여지지 않는다", dim: true},
                {mark: "✕", text: "실사용자 규모에서의 동작은 확인된 바 없다", dim: true},
              ].map((row) => (
                <li
                  key={row.text}
                  className="flex items-start gap-2 text-[15px] leading-[32px]"
                  style={{color: row.dim ? "rgba(255,255,255,0.40)" : "rgba(255,255,255,0.88)"}}
                >
                  <span className="mt-[2px] select-none" style={{color: row.dim ? "rgba(255,255,255,0.40)" : "#4ade80"}}>
                    {row.mark}
                  </span>
                  <span>{row.text}</span>
                </li>
