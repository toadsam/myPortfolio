...
const MOTIFS = [
  ...
  <path d="M10 50 Q 30 20 50 50 T 90 50" fill="none" stroke="currentColor" strokeWidth="4" />,
  <>
    <rect x="25" y="25" width="20" height="20" fill="currentColor" />
    <rect x="55" y="55" width="20" height="20" fill="currentColor" />
    <rect x="55" y="25" width="20" height="20" fill="currentColor" opacity="0.3" />
    <rect x="25" y="55" width="20" height="20" fill="currentColor" opacity="0.3" />
  </>,
  <>
...
export function HeroSection({start}: Props) {
  ...
  const bgPosters = useMemo(
    () =>
      CLUBS.slice(0, 24).map((club, i) => ({
        name: club.name,
        cat: CATEGORY_NAMES[Math.floor(seeded(i + 11) * CATEGORY_NAMES.length)]!,
  ...
  useEffect(() => {
    ...
    bgPosters.forEach((_, i) => push(() => setPostersIn((n) => Math.max(n, i + 1)), i * 20));
    push(() => setPhase(1), 300); // 키커
    headWords.forEach((_, i) => push(() => setWords1((n) => Math.max(n, i + 1)), 500 + i * 40));
    push(() => setPhase(2), 1050); // 두 번째 줄
    push(() => setPhase(3), 1500); // 요약
    for (let i = 0; i < 5; i += 1) push(() => setCellsIn((n) => Math.max(n, i + 1)), 1950 + i * 90);
    for (let i = 0; i < 3; i += 1) push(() => setTrioIn((n) => Math.max(n, i + 1)), 2400 + i * 180);
  ...
  return (
    ...
    >
      {/* 벽에 남은 테이프 자국 */}
      <div className="absolute inset-x-0 top-0 z-0 h-[1px] bg-gradient-to-b from-[#1a1130] to-transparent" aria-hidden="true">
      ...
      {/* ── 배경 포스터 벽 ── */}
      <div className="ac-wall-mask absolute inset-0 z-0 overflow-hidden" aria-hidden="true">
        <div
          ...
          {bgPosters.map((p, i) => {
            ...
            return (
              <div
                ...
                style={{
                  opacity: shown ? 1 : 0,
                  transform: `translate(${p.tx}px, ${p.ty}px) rotate(${p.tilt}deg) scale(${shown ? 1 : 0.85})`,
                  transition: reducedMotion
                    ? "none"
                    : "transform 0.5s cubic-bezier(0.22,1,0.36,1), opacity 0.3s linear",
                }}
              >
                <div className="ac-cat-bar absolute inset-x-0 top-0 h-[4px] transition-shadow duration-300" style={{background: color}} />
                ...
                <div className="flex flex-grow flex-col justify-between p-3 pt-4">
                  <div>
                    <div className="text-[13px] font-black leading-snug text-[rgba(255,255,255,0.88)]">{p.name}</div>
                    <div className="mt-1 font-mono text-[9px] tracking-wider text-[rgba(255,255,255,0.46)]">{p.cat}</div>
                  </div>
                  <div className="relative h-[60px] w-full opacity-40" style={{color}}>
                    <svg className="absolute bottom-0 right-0 h-[40px] w-[40px]" viewBox="0 0 100 100">
                      {p.motif}
                    </svg>
                  </div>
                </div>
      ...
      <div className="relative z-10 mt-12 flex w-full max-w-[720px] flex-col items-center px-6 md:mt-24">
        ...
        <h2 className="flex flex-col items-center text-center text-[27px] font-black leading-tight text-[rgba(255,255,255,0.88)] md:text-[42px]">
          ...
          <span
            ...
            style={{opacity: phase >= 2 ? 1 : 0, transform: phase >= 2 ? "none" : "translateY(10px)"}}
          >
            {HEADLINE_2}
          </span>
        </h2>
        ...
        >
          동아리 24개를 한 벽에 붙여놓고, <strong className="font-bold text-[#d8b4fe]">조건을 누르면 남는 것만 남게 했다.</strong>
        ...
        <div className="mt-[30px] grid w-full max-w-[700px] grid-cols-2 gap-[16px] md:grid-cols-4">
          {META.map((cell, i) => (
            <div
              ...
              style={{
                border: `1px solid ${cell.warn ? "rgba(251,191,36,0.30)" : "rgba(192,132,252,0.20)"}`,
      ...
      <div className="relative z-20 mt-16 flex w-full max-w-[900px] flex-col items-center justify-center gap-[24px] px-4 md:mt-24 md:flex-row md:gap-[32px]">
        <button
          ...
          style={{
            ...
            transform: trioIn >= 1 ? `rotate(${trio[0]!.rotate}) scale(1)` : "translateY(30px) rotate(-10deg) scale(0.95)",
            transition: "opacity 0.5s cubic-bezier(0.22,1,0.36,1), transform 0.5s cubic-bezier(0.22,1,0.36,1)",
          }}
        >
          <span className={`ac-tape ${trioIn >= 1 ? "ac-in" : ""} absolute left-[10px] top-[-6px] z-10 h-[10px] w-[22px] rotate-[45deg] bg-[rgba(255,255,255,0.08)] backdrop-blur-sm`} />
          <span className={`ac-tape ${trioIn >= 1 ? "ac-in" : ""} absolute right-[10px] top-[-6px] z-10 h-[10px] w-[22px] -rotate-[45deg] bg-[rgba(255,255,255,0.08)] backdrop-blur-sm`} />
          <span className="relative flex w-full flex-grow items-center justify-center overflow-hidden rounded-sm border border-[rgba(255,255,255,0.05)] bg-[rgba(0,0,0,0.3)]">
            <span className="flex h-[42px] w-[42px] items-center justify-center rounded-full border border-[rgba(216,180,254,0.45)] transition-colors group-hover:bg-[rgba(216,180,254,0.1)] md:h-[56px] md:w-[56px]">
              <svg className="ml-1 h-[20px] w-[20px] text-[#d8b4fe] md:h-[26px] md:w-[26px]" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
          ...
          <span className="mt-3 flex w-full flex-col gap-1">
            <span className="text-[13px] font-black text-[#d8b4fe] md:text-[15px]">데모 영상</span>
            <span className="font-mono text-[9px] text-[rgba(255,255,255,0.55)] md:text-[10px]">1분 54초</span>
          </span>
        </button>
        ...
        <a
          ...
          style={{
            ...
            transform: trioIn >= 2 ? `rotate(${trio[1]!.rotate}) scale(1)` : "translateY(30px) rotate(-10deg) scale(0.95)",
            transition: "opacity 0.5s cubic-bezier(0.22,1,0.36,1), transform 0.5s cubic-bezier(0.22,1,0.36,1)",
          }}
        >
          <span className={`ac-tape ${trioIn >= 2 ? "ac-in" : ""} absolute left-[10px] top-[-6px] z-10 h-[10px] w-[22px] rotate-[45deg] bg-[rgba(255,255,255,0.08)] backdrop-blur-sm`} />
          <span className={`ac-tape ${trioIn >= 2 ? "ac-in" : ""} absolute right-[10px] top-[-6px] z-10 h-[10px] w-[22px] -rotate-[45deg] bg-[rgba(255,255,255,0.08)] backdrop-blur-sm`} />
          ...
          <span className="z-10 mt-3 flex w-full flex-col gap-1">
            <span className="text-[13px] font-black text-[rgba(255,255,255,0.88)] md:text-[15px]">GitHub 저장소</span>
        ...
        <div
          ...
          style={{
            ...
            transform: trioIn >= 3 ? `rotate(${trio[2]!.rotate}) scale(1)` : "translateY(30px) rotate(-10deg) scale(0.95)",
            transition: "opacity 0.5s cubic-bezier(0.22,1,0.36,1), transform 0.5s cubic-bezier(0.22,1,0.36,1)",
          }}
        >
          <span className={`ac-tape ${trioIn >= 3 ? "ac-in" : ""} absolute left-[40%] top-[-6px] z-10 h-[10px] w-[22px] -rotate-[5deg] bg-[rgba(255,255,255,0.1)] backdrop-blur-sm`} />
      ...
      {videoOpen ? (
        ...
        >
          ...
          <div className="relative flex aspect-video w-full max-w-[1020px] items-center justify-center border border-[rgba(192,132,252,0.2)] bg-[#08040e] shadow-2xl">
            <span className="font-mono text-sm tracking-widest text-[rgba(255,255,255,0.4)]">데모 영상 자리 · 16:9</span>
          </div>
        </div>
      ) : null}
    </section>
