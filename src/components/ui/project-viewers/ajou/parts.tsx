...
// 여러 페이지가 공유하는 조각들.
// 색·크기·이징은 portfolio-specs/08-ajou-adventure.md 의 토큰을 그대로 따른다.
 ...
 * 스크롤을 막지 않는다 — 스크롤을 뺏지 않는다 - 이 방은 관람객이 조작하는 방이다.
 */
      ...
      {text.split(" ").map((word, i) => (
        <span
          ...
          style={{
            ...
            transition: instant
              ? "none"
              : `opacity 0.3s var(--aj-ease) ${
                  (i * 30) / 1000
                }s, transform 0.3s var(--aj-ease) ${(i * 30) / 1000}s`
          }}
        >
          {word}
        </span>
...
export function Body({
  ...
  return (
    <p
      className={`max-w-[62ch] text-[15px] leading-8 text-[var(--aj-text)] sm:text-[16px] ${className}`}
    ...
    >
      {label ? (
        <div className="mb-3 flex items-center justify-between gap-2">
          <span className="font-mono text-[10px] tracking-[0.18em] text-[var(--aj-muted)]">
            {label}
          </span>
          {right}
        </div>
...
export function MetaCell({value, label}: {value: string; label: string}) {
  return (
    ...
    >
      <div className="font-mono text-[20px] font-black text-[var(--aj-text)] sm:text-[24px]">
        {value}
      </div>
      <div className="mt-1 font-mono text-[11px] tracking-[0.1em] text-[var(--aj-muted)]">
        {label}
      </div>
    </div>
  );
...
export function CodePanel({
  ...
  return (
    <div
      ...
      style={{border: `1px solid ${borderColor}`, background: "var(--aj-code-bg)"}}
    >
      <div
      ...
      >
        <span className="min-w-0 flex-1 truncate font-mono text-[11px] text-[var(--aj-muted)]">
          {filename}
        </span>
        {badge ? (
          <span
...
export function CodeLine({
  ...
  const accent = bad ? "var(--aj-bad)" : "var(--aj-primary)";
  ...
  return (
    ...
    >
      <span className="w-5 shrink-0 select-none text-right text-[var(--aj-faint)]">
        {n}
      </span>
...
export function Cm({children}: {children: ReactNode}) {
  return <span style={{color: "var(--aj-c-comment)"}}>{children}</span>;
}
  ...
  return (
    <div>
      <div className="mb-1.5 flex items-baseline justify-between gap-2">
        <span className="font-mono text-[10px] tracking-[0.14em] text-[var(--aj-muted)]">
          {label}
        </span>
        <span
      ...
      {hint ? (
        <p className="mt-1 font-mono text-[10px] leading-4 text-[var(--aj-faint)]">
          {hint}
        </p>
      ) : null}
    </div>
...
export function Switch2({
  ...
  return (
    <div
      ...
      style={{border: "1px solid var(--aj-border)"}}
      role="group"
      aria-label={label}
    >
      {options.map((opt, i) => {
        ...
        return (
          <button
            ...
            style={{
              background: on ? "rgba(163,230,53,0.20)" : "transparent",
              color: on ? "var(--aj-primary)" : "var(--aj-muted)"
            }}
          >
            {opt}
          </button>
  ...
  return (
    ...
    >
      <span className="min-w-0">
        <span className="block text-[13px] font-bold text-[var(--aj-text)]">
          {title}
        </span>
...
export function Hint({children}: {children: ReactNode}) {
  return (
    <p className="font-mono text-[11px] leading-5 text-[var(--aj-faint)]">
      {children}
    </p>
  );
}
...
export function Caveat({children}: {children: ReactNode}) {
  return (
    <div className="mt-4 font-mono text-[10px] leading-5 text-[var(--aj-faint)]">
      {children}
    </div>
  );
}
...
export function Shot({
  ...
  const inner = src ? (
    ...
    <div className="flex h-full w-full flex-col items-center justify-center gap-2 bg-[#101707]">
      <div className="flex flex-col items-center gap-1" aria-hidden="true">
        <span className="block h-2.5 w-24 rounded-sm bg-[#22300f]" />
        <span className="block h-2.5 w-16 rounded-sm bg-[#1b270b]" />
        <span className="block h-2.5 w-28 rounded-sm bg-[#22300f]" />
      </div>
  ...
  return (
    ...
    >
      {onOpen && src ? (
        ...
        >
          ...
          <span
            ...
            style={{background: "rgba(0,0,0,0.6)", color: "var(--aj-accent)"}}
          >
            ⤢ 확대
          </span>
        </button>
      ...
      <figcaption
        className="px-3 py-2 font-mono text-[11px] text-[var(--aj-muted)]"
  ...
  return (
    <div
      ...
      style={{
        ...
        transition: instant
          ? "none"
          : `opacity 0.3s var(--aj-ease) ${delay}ms`
      }}
    >
      <span className="w-[62px] shrink-0 font-mono text-[11px] text-[var(--aj-muted)]">
        {label}
      </span>
      <div className="aj-bar flex-1">
        <span
          style={{
            width: on ? `${pct}%` : "0%",
      ...
      <span className="w-[52px] shrink-0 text-right font-mono text-[11px] tabular-nums text-[var(--aj-accent)]">
        {value}
      </span>
    </div>
  );
...
export function EquipSlot({
  ...
  const body = (
    <>
      ...
      >
        {equipped && !instant ? (
          <span
            className="aj-popring absolute rounded-full"
            style={{
      ...
      <span
        className={equipped && !instant ? "aj-pop" : ""}
  ...
  const boxStyle: CSSProperties = {
    border: equipped
      ...
      : "1px dashed rgba(163,230,53,0.26)",
    ...
    <div className="flex flex-col items-center gap-2">
      {href ? (
        <a
          ...
          className={`${boxClass} cursor-pointer hover:-translate-y-[3px] hover:border-[rgba(163,230,53,0.7)]`}
          style={boxStyle}
      ...
      <span
        ...
        style={{color: note ? "var(--aj-muted)" : "var(--aj-accent)"}}
      >
        {caption}
      </span>
    </div>
  ...
  return (
    ...
    >
      <div
        ...
        style={{color: lit ? color : "var(--aj-muted)"}}
      >
        {done ? "✓ " : ""}
        {title}
      </div>
      {sub ? (
        <div className="mt-1 font-mono text-[10px] leading-4 text-[var(--aj-faint)]">
          {sub}
        </div>
      ) : null}
    </div>
...
export function LogLine({
  ...
  const ok = status < 400;
  const color = ok ? "var(--aj-ok)" : "var(--aj-bad)";
  return (
    <div className="flex items-start gap-2 px-3 py-1 font-mono text-[11px] leading-5">
      <span className="shrink-0 text-[var(--aj-faint)]">{time}</span>
      <span className="shrink-0 text-[var(--aj-muted)]">{method}</span>
      <span className="min-w-0 flex-1 truncate text-[var(--aj-text)]">
        {path}
      </span>
