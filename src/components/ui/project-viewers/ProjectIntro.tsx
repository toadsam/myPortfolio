...
export function ProjectIntro({title, theme, variant}: {title: string; theme: ProjectTheme; variant: AmbientVariant}) {
      ...
      {show ? (
        ...
        >
          ...
          <motion.div
            ...
            style={{background: `radial-gradient(circle at 50% 50%, ${theme.primary}1a, transparent 60%)`}}
            animate={horror ? {opacity: [0.2, 0.05, 0.3, 0.1]} : {opacity: [0.1, 0.3, 0.1]}}
            transition={{duration: horror ? 0.4 : 2, repeat: Infinity, repeatType: "mirror"}}
          />
          ...
          <motion.h1
            ...
            initial={{opacity: 0, scale: horror ? 1.1 : 0.85, filter: "blur(8px)"}}
            animate={
              horror
                ? {opacity: [0, 1, 0.4, 1], scale: 1, filter: "blur(0px)", x: [0, -4, 4, -2, 0]}
          ...
          <motion.p
            ...
            transition={arcade ? {duration: 0.9, repeat: Infinity, delay: 0.6} : {duration: 0.5, delay: 0.7}}
          >
            {copy.sub}
          </motion.p>

          {/* 로딩 바 */}
          <motion.div className="mt-8 h-0.5 w-48 overflow-hidden rounded-full" style={{background: "rgba(255,255,255,0.1)"}}>
            <motion.div className="h-full" style={{background: theme.primary}} initial={{width: "0%"}} animate={{width: "100%"}} transition={{duration: 2, ease: "easeInOut"}} />
          </motion.div>
