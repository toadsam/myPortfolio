...
const MODE_COPY: Record<
  CineMode,
  {label: string; status: string; color: string; duration: string; ease: string}
> = {
  manual: {
    label: "직접 Lerp 구현",
    status: "시작과 끝이 툭 끊긴다",
    ...
    >
      <div className="z-10 w-full max-w-[900px] px-6">
        <header className="dl-fade-up mb-20">
          ...
          <h2 className="mb-10 mt-4 text-4xl font-black tracking-tight md:text-[44px]">
            처음엔 카메라를 직접 움직였다
          </h2>
          <div className="space-y-6 text-lg leading-8 text-[rgba(255,255,255,0.74)]">
            <p>
              Lerp로 회전시키고, 다 되면 제어권을 돌려주는 식으로 짰다. 동작은
              했다. 문제는{" "}
              <span className="font-bold text-[#ff5a4d]">
                전환이 어색했다는 것이다.
              </span>{" "}
              시작할 때 급하게 꺾이고, 끝날 때 툭 끊겼다.
            </p>
            <p>
              이징 곡선을 손으로 만지면서 며칠을 썼다. 연출마다 다른 곡선이
              필요했고, 그걸 전부 코드로 들고 있어야 했다.
            </p>
            <p>
              Cinemachine의 가상 카메라로 바꾸고 나서는 어디를 보게 할지만
              정하면 블렌딩이 알아서 처리됐다. 연출 하나 만드는 시간이 절반
              이하로 줄었다.
            </p>
          </div>
        </header>
        ...
        <div className="dl-fade-up mb-24 overflow-hidden rounded-md border border-[rgba(255,255,255,0.10)] bg-[#0b0708] shadow-2xl">
          ...
          <div className="bg-[#090607] px-8 py-6">
            <div className="relative mb-3 h-1 rounded-full bg-white/10">
              <div
                ref={trackRef}
                className="absolute left-0 top-0 h-full w-0 bg-[#ff5a4d]/30"
              />
            </div>
            <div
