...
export const hero: HeroContent = {
  ...
  // 세 줄을 「무엇을 쓸 줄 아는가」에서 「무엇이 실제로 일어났는가」로 바꿨다.
  //
  // 예전 세 줄은 전부 기술 나열이었다(React·Spring Boot / JWT·CORS / Unity XR).
  // 그런데 그건 바로 아래 「기술 스택(02)」 절이 표로 통째로 말한다 — 히어로에서
  // 두 번 말하느라, 정작 이 이력서에서 가장 센 사실 셋(3,500명이 쓴다 / 사용자
  // 말을 듣고 다시 만들었다 / 배포 뒤 문제를 닫았다)이 넷째 화면까지 밀려 있었다.
  //
  // 셋 다 아래 카드에 근거가 이미 있는 문장이다. 새로 주장하는 것이 없다:
  //   1번 → aClub·총학 카드 지표(GA4 / Search Console)
  //   2번 → 득근득근 카드 「3 / 4 반영한 사용자 피드백」
  //   3번 → 전용 전시실 P03Refresh·P06Cors (토큰 재발급 단일화 · CORS)
  // 기술 이름은 3번에만 남겼다. 겪은 문제를 말하려면 이름이 필요해서다.
  bullets: [
    "만든 것이 실제로 쓰입니다 — aClub 활성 사용자 3,500명 · 총학생회 웹 검색 노출 34,200",
    "쓰는 사람 말을 듣고 고칩니다 — 득근득근 피드백 4건 중 3건 반영 · TSEROF 테스터 4건 전부 조치",
    "배포 뒤에 생기는 문제까지 닫습니다 — 토큰 재발급 경쟁 상태 · HTTPS/CORS · SSE"
  ]
};
