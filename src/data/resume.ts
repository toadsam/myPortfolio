...
/**
 * 인쇄 전용 요약. 화면(`ResumeMode`)은 쓰지 않는다 — PDF 빌더만 읽는다.
 *
 * 2026-09-12, 8쪽을 줄이면서 `{head, body}` 두 줄 구조를 **한 줄씩 네 개**로
 * 바꿨다. body 는 head 를 풀어 쓴 문장이라 한 항목이 2~3줄을 먹었는데(실측 9줄),
 * 정작 심사자가 집어 가는 건 body 안의 고유명사와 기술 이름이었다. 그래서 문장은
 * 버리고 **그 이름들만 괄호로 끌어올렸다**. lead 도 두 줄에서 한 줄로 줄였다 —
 * 다만 지우지는 않는다. 바로 위 제목 블록은 직무와 포부만 말하고, "React ·
 * Spring Boot" 라는 기술 좌표를 처음 주는 자리가 여기다.
 */
export const printSummary = {
  lead: "React · Spring Boot 중심 웹 풀스택. 인증/보안과 배포 운영 이슈까지 직접 다룹니다.",
  points: [
    "화면부터 API · 인증 · DB · 배포까지 한 흐름으로 구현합니다 (FestFlow · 총학생회 웹)",
    "배포 후 드러나는 문제를 재현해 고칩니다 (HTTPS · CORS credentials · 토큰 재발급 경쟁 상태)",
    "실사용 피드백과 GA4/GSC 지표를 근거로 고칩니다 (aClub · 총학생회 웹)",
    "AI 코딩 도구가 낸 값은 직접 측정해 확인하고 씁니다"
  ]
};
...
export const skillDetails: SkillDetail[] = [
  ...
  },
  {
    ...
    //
    // 2026-09-12 — 칩에 "(적용 후 롤백)" 을 붙였다. 위 판단은 그대로 맞지만,
    // **칩만 훑는 사람에게는 "지금 그렇게 돌고 있다" 로 읽힌다.** 실제 코드는 단일
    // refresh token 유지 + 클라이언트 재발급 단일화이고(면접대비 문서에도 그렇게
    // 적혀 있다), 그 상태에서 "Rotation 구현하셨죠" 를 받으면 해명부터 해야 한다.
    // 경험은 남기고 오해만 없앤다 — 적용과 롤백의 전말은 득근득근 항목 본문에 이미
    // 한 문장으로 있다.
    desc: "JWT 인증/인가 · 이중 쿠키 + 재발급 단일화",
    core: ["JWT"],
    stack: [
      "JWT",
      "Refresh Token Rotation (적용 후 롤백)",
      "OAuth2",
      "Passport",
      "Firebase Auth"
    ]
...
export const careers: CareerItem[] = [
  ...
  // 스탬프고, 본인 확인 결과 **지금도 재직 중**이다(2026-08-25).
  {
    ...
    // 2026-09-12 "재직 중" → "활동 중". 사실(지금도 맡고 있다)은 위 주석 그대로이고
    // 말만 고쳤다 — 학생자치 기구는 재직하는 곳이 아니라, 인사담당이 경력 용어의
    // 부정확으로 읽는다.
    period: "2024.03 ~ 활동 중",
