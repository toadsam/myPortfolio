...
async def answer_with_openai(npc: dict[str, Any], context: str, message: str) -> str:
    ...
    system_prompt = (
        ...
        "감정과 기억이 있는 것처럼 행동하되, 사실은 제공된 포트폴리오 데이터와 최근 대화만 근거로 삼는다. "
        "모르는 내용은 지어내지 말고 확인 가능한 범위에서 답한다. "
        "방문자가 채용자라면 강점, 대표 프로젝트, 기술 판단, 협업 가능성을 명확하게 정리한다. "
        "답변은 한국어로 3~7문장, 필요하면 짧은 목록으로 한다. "
        "브라우저 대화창에 그대로 표시되므로 마크다운 굵게, 제목 문법, 코드블록은 쓰지 않는다. "
        "NPC의 성격, 감정 경향, 현재 목표를 말투에 반영한다.\n"
        f"NPC 역할: {_profile_text(npc, 'role', '')}\n"
        f"말투: {_profile_text(npc, 'tone', '')}\n"
        f"답변 범위: {_profile_text(npc, 'scope', '')}\n"
        f"컨텍스트:\n{context}"
    )
...
def answer_without_ai(npc_id: str, message: str, activity: DailyActivity) -> str:
    ...
    if _contains(message, ["대표", "추천", "best", "main"]):
        return (
            ...
            "근근 MuscleUp은 운동 기록, 커뮤니티, 인증, SSE, AI 분석을 하나의 풀스택 흐름으로 묶은 점이 강합니다."
        )

    if _contains(message, ["강점", "요약", "strength", "장점"]):
        return (
            "정재훈의 강점은 세 가지로 정리돼요. 첫째, 문제를 기능 목록이 아니라 사용자 흐름으로 다시 구조화합니다. "
            "둘째, React/TypeScript 프론트엔드와 Spring Boot/FastAPI 백엔드를 함께 다루며 실제 서비스 흐름을 만듭니다. "
            "셋째, Three.js와 Unity 경험이 있어 포트폴리오 자체도 3D 인터랙션으로 설계할 만큼 표현력이 있습니다."
        )

    if project:
        return _project_answer(project)

    if _contains(message, ["기술", "스택", "stack", "개발", "아키텍처"]):
        return (
            "주요 기술은 React, TypeScript, Next.js, Three.js/R3F, FastAPI, Spring Boot, Unity/C#입니다. "
            "프론트엔드는 화면 상태와 사용자 흐름을 설계하는 쪽이 강하고, 백엔드는 인증, API, 실시간 SSE 같은 서비스 기능을 연결한 경험이 있습니다. "
            "특히 이 포트폴리오 마을은 Next.js 화면, Three.js 3D 씬, FastAPI 라이브 상태, NPC 대화 API가 함께 연결된 구조예요."
        )

    if _contains(message, ["협업", "팀", "소통", "역할", "collaboration"]):
        return (
            "협업 관점에서는 역할을 화면 단위와 데이터 흐름 단위로 쪼개는 편이에요. "
            "ACLUB, 아주총학, FestFlow 같은 프로젝트에서는 사용자 흐름, 권한, 배포 구조를 나눠 생각한 흔적이 있고, "
            "Unity 프로젝트에서는 충돌을 줄이기 위해 기능 단위로 작업 범위를 분리한 경험이 있습니다."
        )

    if _contains(message, ["오늘", "상태", "활동", "today", "커밋", "공부", "운동"]):
        workout = "운동도 완료되어 마을 분위기가 더 활기차요" if activity.workout_done else "운동 기록은 아직 없어서 광장 쪽 에너지는 차분해요"
        ...
        return (
            ...
            f"{workout}.{memo} 이 값은 건물 조명, NPC 기분, 해금 장식에 반영돼서 방문자가 현재 컨디션을 볼 수 있게 설계되어 있어요."
        )

    if _contains(message, ["연락", "메일", "github", "contact", "채용", "문의"]):
        return (
            "연락은 toadsam@naver.com 으로 보낼 수 있고, GitHub는 https://github.com/toadsam 입니다. "
            "채용자 관점에서는 MyWave, FestFlow, 근근 MuscleUp을 먼저 보고 기술 질문은 테오에게 이어서 물어보는 흐름이 가장 빠릅니다."
        )

    if npc_id == "developer-npc" or "skill" in npc_id:
        return (
            "기술 질문이라면 프로젝트와 연결해서 보는 게 좋아요. MyWave는 React/TypeScript 기반 정보 구조화, "
            "FestFlow는 Spring Boot와 SSE 실시간 흐름, 이 포트폴리오는 Next.js와 Three.js/FastAPI 연결 경험을 보여줍니다."
        )

    if npc_id == "archivist-npc" or "exp" in npc_id:
        return (
            "기록 관점에서 보면 정재훈은 프로젝트 결과만 놓는 대신 문제, 접근, 기여, 배운 점을 같이 남기려는 편이에요. "
            "이 흐름이 포트폴리오 상세 페이지와 오늘의 마을 상태에 연결되어 있습니다."
        )

    if npc_id == "contact-npc" or "post" in npc_id:
        return (
            "다음 행동은 간단해요. GitHub에서 코드를 보고 싶다면 github.com/toadsam, 직접 연락하려면 toadsam@naver.com 으로 이어가면 됩니다. "
            "관심 프로젝트를 같이 적어주면 대화가 더 빠르게 이어질 거예요."
        )
