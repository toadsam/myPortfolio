...
뜨는 문장: _"방금 하신 게, 제 첫 버전에서는 실제로 통했습니다."_
...
> 이벤트 `lounge:join` `lounge:welcome` `lounge:players` `player:move` `chat:send` `chat:typing` > `social:emote` `social:sticker` `party:follow-request` `friend:send` 등 ·
