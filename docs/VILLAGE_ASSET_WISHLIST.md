...
| 어디           | 남은 것                                                                                        |
| -------------- | ---------------------------------------------------------------------------------------------- |
| 건물 NPC **4** | 까치(`exp-unity-ui`)·수달(`exp-demo-platform`)·여우(`exp-portfolio`)·나무늘보(`life-timeline`) |
| **플레이어 1** | 정재훈 본인 아바타 — 걷기 모드가 아직 **레드실드 전사**를 그린다                               |

- **EXPERIENCE 구역은 3채가 통째로 로봇이다** — 남은 건물 NPC 4명 중 셋이 그 구역이라,
  한 구역만 보면 캐스팅이 안 된 티가 제일 크게 난다.
- **걷기 모드 조종 캐릭터**는 `warrior-walk.glb`(Meshy 레드실드 전사)다. 프롬프트는
  [`NPC_CHARACTER_PROMPTS.md`](NPC_CHARACTER_PROMPTS.md) 의 「플레이어 (1)」 절에 있다.
  `jaehoon.glb` 에 이미 walking/running 이 있어 **코드 두 줄로 임시 교체도 된다**
  (단 광장 총괄 NPC 와 같은 모델이 되어 자기 자신과 마주친다).
