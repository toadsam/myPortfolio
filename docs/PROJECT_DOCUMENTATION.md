...
### 12.4 첫 방문 로딩 순서 (2026-09-20)

위 스니펫은 구조를 보여 주려고 단순하게 줄인 것입니다. 지금 코드에는 `AdaptiveDpr` 와 `AdaptiveEvents` 가 없고,
로딩 순서를 관리하는 `SceneWarmup` 이 들어 있습니다.

그래픽카드는 재질마다 있는 셰이더(표면을 어떻게 칠할지 적은 작은 프로그램)를 처음 볼 때 한 번 번역해야 합니다.
첫 방문에는 번역해 둔 것이 없어서 이 시간이 깁니다. 예전에는 건물이 도착해 처음 그려질 때마다 하나씩 번역했고,
그동안 화면이 멈췄습니다. 첫 방문에서 타이틀이 뜨기까지 약 20초가 걸렸습니다.

지금 순서는 다음과 같습니다.

```text
1. 소품·건물·주민 묶음이 각자 도착하면 GroupProbe 가 신호를 올린다
2. 셋이 다 올 때까지 Canvas 는 frameloop="never" (아무것도 그리지 않는다)
3. 다 오면 gl.compileAsync 로 셰이더를 한꺼번에, 멈춤 없이 번역한다
4. 그리기를 켜고 보이지 않는 프레임 4장을 돌린다
5. markVillageReady → 로딩 액자가 걷히고 "눌러서 들어가기" 가 나온다
```

같은 조건에서 약 6초로 줄었습니다. 주의할 점은 세 가지입니다.

- `shadows="percentage"` 를 `shadows={true}` 로 되돌리면 안 됩니다. 미리 번역한 셰이더와 실제로 쓰는 셰이더가 달라져서 두 번 번역합니다.
- 조건에 따라 나타났다 사라지는 `<pointLight>` 를 새로 달면 안 됩니다. 광원 개수가 바뀌면 전체를 다시 번역합니다. `PooledLight` 를 씁니다.
- 제대로 되고 있는지는 화면으로 알 수 없습니다. 측정 방법과 수치는 `docs/VILLAGE_OVERHAUL_REPORT.md` §6-b 에 있습니다.
...
#### 키보드로 날아다니기 (2026-09-20)

클릭 모드에서 `W` `A` `S` `D`(또는 방향키)를 누르면 카메라가 보는 방향으로 날아갑니다. `Q` `E` 는 아래와 위, `Shift` 는 가속,
마우스 오른쪽 버튼을 누른 채 움직이면 시선이 돌아갑니다. 원래는 개발할 때만 켜지던 기능인데 배포본에도 열었습니다.

```text
키를 누른다 -> enterFree() : OrbitControls 를 끄고 카메라를 직접 움직인다
키를 뗀다   -> exitFree()  : 지금 보는 방향 앞에 회전 중심을 놓고 OrbitControls 를 다시 켠다
```

다음 상황에서는 부모(`AIPortfolioVillage`)가 `flyEnabled={false}` 를 내려 보내 꺼 둡니다.

- 마을에 들어오기 전, 루미가 달려오는 환영 연출 중
- NPC 대화창, 정보 패널, 엿듣기가 열려 있을 때 (방향키로 패널을 스크롤해야 하기 때문입니다)

이동 범위는 x·z ±48, 높이 2~50 으로 막혀 있습니다. 직접 이동 모드(15.2)에서는 같은 키가 캐릭터를 움직입니다.
